...
function countIssueSeverities(issues) {
  ...
  const errorCount = safeIssues.filter(
    (issue) => normalizeIssueSeverity(issue?.severity) === "error",
  ).length;
  const warningCount = safeIssues.filter(
    (issue) => normalizeIssueSeverity(issue?.severity) === "warning",
  ).length;
...
function resolveShowWarningsFromReportData(reportData) {
  const explicitShowWarnings =
    reportData?.executionProfile?.display?.showWarnings;
...
function renderObjectPassFailCountMarkup(passedObjectCount, failedObjectCount) {
  const safePassedCount = Math.max(
    0,
    Number.isFinite(passedObjectCount) ? passedObjectCount : 0,
  );
  const safeFailedCount = Math.max(
    0,
    Number.isFinite(failedObjectCount) ? failedObjectCount : 0,
  );
...
None
*/
function renderBreakdownMetricRow(
  label,
  correctCount,
  incorrectCount,
  totalCount,
...
None
*/
function renderScopeBreakdownTable(
  cardTitle,
  descriptionItems,
  scopeBreakdown,
) {
  ...
  const identificationEnabled =
    scopeEnabled && safeBreakdown.identificationEnabled !== false;
  ...
  const contentIncorrectCount = Number(
    safeBreakdown.contentIncorrectCount || 0,
  );
  ...
  return safeFindings.reduce(
    (accumulator, finding) => {
      ...
      return {
        ...
        unidentified:
          accumulator.unidentified + (finding?.isUnidentified ? 1 : 0),
      };
    },
...
function renderTopErrorSummaryTable(reportData, showWarnings) {
  ...
  const productTotals = calculateScopeTotals(
    productScopeEnabled ? reportData?.productFindings : [],
  );
  const systemTotals = calculateScopeTotals(
    systemScopeEnabled ? reportData?.systemFindings : [],
  );
  const warningHeaderMarkup = showWarnings ? "<th>Warnings</th>" : "";
  const productWarningCellMarkup = showWarnings
    ? `<td>${productScopeEnabled ? productTotals.warnings : "-"}</td>`
    : "";
  const systemWarningCellMarkup = showWarnings
    ? `<td>${systemScopeEnabled ? systemTotals.warnings : "-"}</td>`
    : "";
...
function renderSeverityFindingTable(findingsBySeverity, severityKind) {
  const safeFindings = Array.isArray(findingsBySeverity)
    ? findingsBySeverity
    : [];
...
None
*/
function renderSeverityIndicatorButton(
  buttonKind,
  countValue,
  encodedPayload,
  finding,
...
function renderErrorColumnContent(errorCount, errorPayload, finding) {
  const errorIndicatorMarkup = renderSeverityIndicatorButton(
    "error",
    errorCount,
    errorPayload,
    finding,
...
function renderObjectRow(finding, scopeKey, groupKey, showWarnings) {
  ...
  const searchText = escapeHtml(
    buildObjectSearchText(finding, scopeKey, groupKey),
  );
  const objectGlobalId = String(finding?.globalId || "-");
  const issueRows = Array.isArray(finding?.issues) ? finding.issues : [];
  const errorRows = issueRows.filter(
    (item) => normalizeIssueSeverity(item?.severity) === "error",
  );
  const warningRows = issueRows.filter(
    (item) => normalizeIssueSeverity(item?.severity) === "warning",
  );
  ...
  const errorColumnContent = renderErrorColumnContent(
    errorCount,
    errorPayload,
    finding,
  );
  const warningColumnContent = showWarnings
    ? renderSeverityIndicatorButton(
        "warning",
        warningCount,
        warningPayload,
        finding,
...
function sortFindingsForDisplay(findings) {
  return [...(findings || [])].sort((leftFinding, rightFinding) => {
    ...
    const guidCompare = leftGuid.localeCompare(rightGuid, undefined, {
      sensitivity: "base",
    });
    if (guidCompare !== 0) {
      return guidCompare;
...
function groupFindingsByLevelKey(findings) {
  ...
  (findings || []).forEach((finding) => {
    const { normalizedKey, displayKey } = normalizeLevelGroupKey(
      finding?.level3Key,
    );
  ...
  return Array.from(groupMap.values())
    .map((groupValue) => [
      groupValue.displayKey,
      sortFindingsForDisplay(groupValue.items),
    ])
    .sort((leftGroup, rightGroup) => {
      ...
      return String(leftGroup[0]).localeCompare(
        String(rightGroup[0]),
        undefined,
        { sensitivity: "base" },
      );
...
function buildGroupedObjectSeverityRows(groupFindings, severityKind) {
  ...
  return (groupFindings || []).reduce((groupedRows, finding) => {
    const filteredIssues = Array.isArray(finding?.issues)
      ? finding.issues.filter(
          (issue) =>
            normalizeIssueSeverity(issue?.severity) === safeSeverityKind,
        )
      : [];
...
None
*/
function renderGroupSeverityBadgeButton(
  severityKind,
  severityCount,
  payloadKey,
) {
...
function renderGroupBlock(scopeKey, groupKey, groupFindings, showWarnings) {
  const groupTotals = groupFindings.reduce(
    (accumulator, finding) => {
      ...
      return {
        ...
        passedObjectCount:
          accumulator.passedObjectCount + (objectHasErrors ? 0 : 1),
        failedObjectCount:
          accumulator.failedObjectCount + (objectHasErrors ? 1 : 0),
      };
    },
    {
      objectCount: 0,
  ...
  const groupSeverityClass =
    groupTotals.failedObjectCount > 0
      ? "finding-group-has-errors"
      : "finding-group-no-errors";
  const errorGroupedRows = buildGroupedObjectSeverityRows(
    groupFindings,
    "error",
  );
  const warningGroupedRows = showWarnings
    ? buildGroupedObjectSeverityRows(groupFindings, "warning")
    : [];
  const errorPayloadKey =
    errorGroupedRows.length > 0
      ...
      : "";
  const warningPayloadKey =
    showWarnings && warningGroupedRows.length > 0
      ? registerGroupSeverityPayload({
          groupKey,
      ...
      : "";
  const errorObjectGuidList = groupFindings.reduce(
    (guidAccumulator, finding) => {
  ...
  const groupSelectedErrorObjectCount = errorObjectGuidList.filter((ifcGuid) =>
    isBcfIssueSelectionActive(ifcGuid, "error"),
  ).length;
  const isGroupErrorSelectionChecked =
    errorObjectGuidList.length > 0 &&
    groupSelectedErrorObjectCount === errorObjectGuidList.length;

  const objectRows = groupFindings
    .map((finding) =>
      renderObjectRow(finding, scopeKey, groupKey, showWarnings),
    )
    .join("");
  const warningGroupBadgeMarkup = showWarnings
    ? renderGroupSeverityBadgeButton(
        "warning",
        groupTotals.warningCount,
        warningPayloadKey,
      )
    : "";
  const warningHeaderMarkup = showWarnings ? "<th>Warnings</th>" : "";
...
function renderScopeSection(sectionTitle, scopeKey, findings, showWarnings) {
  ...
  const groupMarkup = groupedFindings
    .map(([groupKey, groupItems]) =>
      renderGroupBlock(scopeKey, groupKey, groupItems, showWarnings),
    )
    .join("");
...
function renderFilterToolbar(showWarnings) {
  const warningFilterOptionMarkup = showWarnings
...
function renderGroupedSeverityFindingSections(groupedRows, severityKind) {
  ...
  return safeGroupedRows
    .map((groupedRow) => {
      const objectRows = Array.isArray(groupedRow?.issues)
        ? groupedRow.issues
        : [];
...
function openFindingModal(
  ...
  const findingModalElement = reportListElement.querySelector(
    "[data-finding-modal]",
  );
  const findingModalDialogElement = reportListElement.querySelector(
    ".finding-modal-dialog",
  );
  const findingModalTitleElement = reportListElement.querySelector(
    "#finding-modal-title",
  );
  const findingModalSubtitleElement = reportListElement.querySelector(
    "[data-finding-modal-subtitle]",
  );
  const findingObjectMetaElement = reportListElement.querySelector(
    "[data-finding-object-meta]",
  );
  const findingModalContentElement = reportListElement.querySelector(
    "[data-finding-modal-content]",
  );
  ...
  `;
  findingModalContentElement.innerHTML = renderSeverityFindingTable(
    findingRows,
    safeModalKind,
  );
...
function openGroupFindingModal(reportListElement, groupPayload) {
  ...
  const safeSeverityKind =
    groupPayload?.severityKind === "error" ? "error" : "warning";
  const severityLabel = safeSeverityKind === "error" ? "Errors" : "Warnings";
  const groupedRows = Array.isArray(groupPayload?.groupedRows)
    ? groupPayload.groupedRows
    : [];
  ...
  `;
  findingModalContentElement.innerHTML = renderGroupedSeverityFindingSections(
    groupedRows,
    safeSeverityKind,
  );
  findingModalElement.hidden = false;
...
function collectRenderedSelectableBcfIssueKeys(reportListElement) {
  ...
  const objectRows = Array.from(
    reportListElement.querySelectorAll(".object-row"),
  );
  objectRows.forEach((objectRow) => {
    const rowIfcGuid = decodeDataAttributeValue(
      objectRow.getAttribute("data-object-guid"),
    );
    if (!rowIfcGuid) {
      return;
    }

    const rowErrorCount = Number(
      objectRow.getAttribute("data-error-count") || "0",
    );
    const rowWarningCount = Number(
      objectRow.getAttribute("data-warning-count") || "0",
    );
    const warningSelectionVisible =
      objectRow.querySelector(".warning-indicator-button") !== null;
    ...
    if (rowWarningCount > 0 && warningSelectionVisible) {
      const warningSelectionKey = buildBcfIssueSelectionKey(
        rowIfcGuid,
        "warning",
      );
...
function pruneBcfIssueSelectionsForRenderedReport(reportListElement) {
  const renderedSelectableKeys =
    collectRenderedSelectableBcfIssueKeys(reportListElement);
...
function syncBcfSelectionControls(reportListElement) {
  ...
  const directSelectionCheckboxes = Array.from(
    reportListElement.querySelectorAll('[data-bcf-selection-toggle="true"]'),
  );
  directSelectionCheckboxes.forEach((selectionCheckbox) => {
    ...
    const ifcGuid = decodeDataAttributeValue(
      selectionCheckbox.getAttribute("data-bcf-selection-guid"),
    );
    const severityKind = normalizeBcfSeverityKind(
      selectionCheckbox.getAttribute("data-bcf-selection-severity"),
    );
    const selectionKey = buildBcfIssueSelectionKey(ifcGuid, severityKind);
  ...
  const groupErrorSelectionCheckboxes = Array.from(
    reportListElement.querySelectorAll(
      '[data-bcf-group-selection-toggle="errors"]',
    ),
  );
  groupErrorSelectionCheckboxes.forEach((groupSelectionCheckbox) => {
    ...
    const groupGuidList = parseEncodedGroupGuidList(
      groupSelectionCheckbox.getAttribute("data-bcf-group-guids"),
    );
    ...
    const selectedCount = normalizedGuidList.filter((ifcGuid) =>
      isBcfIssueSelectionActive(ifcGuid, "error"),
    ).length;
    groupSelectionCheckbox.disabled = false;
    groupSelectionCheckbox.checked =
      selectedCount === normalizedGuidList.length;
    groupSelectionCheckbox.indeterminate =
      selectedCount > 0 && selectedCount < normalizedGuidList.length;
  });
}
...
function bindFindingModalControls(reportListElement) {
  ...
  reportListElement.addEventListener("click", (clickEvent) => {
    const selectionClickTarget =
      clickEvent.target instanceof Element
        ? clickEvent.target.closest(
            '[data-bcf-selection-toggle="true"], [data-bcf-group-selection-toggle]',
          )
        : null;
    ...
    if (groupOpenTrigger) {
      ...
      const payloadKey = groupOpenTrigger.getAttribute(
        "data-group-finding-modal-key",
      );
    ...
    if (openTrigger) {
      const findingRows = parseFindingPayload(
        openTrigger.getAttribute("data-finding-modal-payload"),
      );
      const modalKind =
        openTrigger.getAttribute("data-finding-modal-open") === "error"
          ? "error"
          : "warning";
      const objectName =
        decodeDataAttributeValue(
          openTrigger.getAttribute("data-finding-object-name"),
        ) || "Unnamed";
      const objectGlobalId =
        decodeDataAttributeValue(
          openTrigger.getAttribute("data-finding-object-guid"),
        ) || "-";
      const objectIfcEntity =
        decodeDataAttributeValue(
          openTrigger.getAttribute("data-finding-object-ifc"),
        ) || "-";
      const objectExpressId =
        decodeDataAttributeValue(
          openTrigger.getAttribute("data-finding-object-expressid"),
        ) || "-";
  ...
  reportListElement.addEventListener("change", (changeEvent) => {
    const directSelectionCheckbox =
      changeEvent.target instanceof Element
        ? changeEvent.target.closest('[data-bcf-selection-toggle="true"]')
        : null;

    if (directSelectionCheckbox instanceof HTMLInputElement) {
      const ifcGuid = decodeDataAttributeValue(
        directSelectionCheckbox.getAttribute("data-bcf-selection-guid"),
      );
      const severityKind = directSelectionCheckbox.getAttribute(
        "data-bcf-selection-severity",
      );
      setBcfIssueSelectionState(
        ifcGuid,
        severityKind,
        directSelectionCheckbox.checked,
      );
      syncBcfSelectionControls(reportListElement);
      return;
    }

    const groupSelectionCheckbox =
      changeEvent.target instanceof Element
        ? changeEvent.target.closest(
            '[data-bcf-group-selection-toggle="errors"]',
          )
        : null;
...
function bindObjectTableScrollCapture(reportListElement) {
  ...
  reportListElement.addEventListener(
    ...
    (wheelEvent) => {
      ...
      const tableScrollShell = wheelEvent.target.closest(
        "[data-object-table-scroll-shell]",
      );
      ...
      const canScrollDown =
        tableScrollShell.scrollTop + tableScrollShell.clientHeight <
        tableScrollShell.scrollHeight - 1;
...
function normalizeObjectSeverityColumns(reportListElement, showWarnings) {
  ...
  objectRows.forEach((objectRow) => {
    ...
    const firstWarningButton = objectRow.querySelector(
      ".warning-indicator-button",
    );

    const directCellsBeforeNormalization = Array.from(
      objectRow.children,
    ).filter((childElement) => childElement instanceof HTMLTableCellElement);
    ...
    if (firstErrorButton instanceof HTMLElement) {
      ...
      errorCell.innerHTML =
        '<span class="severity-empty-placeholder" aria-hidden="true">-</span>';
    }
    ...
    const rowCells = Array.from(objectRow.children).filter(
...
function applyReportFilters(reportListElement) {
  ...
  const queryInput = reportListElement.querySelector(
    '[data-report-filter="query"]',
  );
  const severitySelect = reportListElement.querySelector(
    '[data-report-filter="severity"]',
  );
  const resultCountElement = reportListElement.querySelector(
    "[data-filter-result-count]",
  );
  const emptyStateElement = reportListElement.querySelector(
    '[data-report-filter="empty"]',
  );

  const normalizedQuery = String(queryInput?.value || "")
    .trim()
    .toLowerCase();
  ...
  objectRows.forEach((objectRow) => {
    const rowSearchText = String(
      objectRow.getAttribute("data-search") || "",
    ).toLowerCase();
    const rowErrorCount = Number(
      objectRow.getAttribute("data-error-count") || "0",
    );
    ...
    const rowIsUnidentified =
      objectRow.getAttribute("data-unidentified") === "true";

    const queryMatch =
      normalizedQuery === "" || rowSearchText.includes(normalizedQuery);
  ...
  const findingGroups = Array.from(
    reportListElement.querySelectorAll(".finding-group"),
  );
  findingGroups.forEach((findingGroup) => {
    const allRowsInGroup = Array.from(
      findingGroup.querySelectorAll(".object-row"),
    );
    const visibleRowsInGroup = allRowsInGroup.filter(
      (objectRow) => !objectRow.hidden,
    );
    const visiblePassedObjects = visibleRowsInGroup.filter(
      (objectRow) =>
        Number(objectRow.getAttribute("data-error-count") || "0") === 0,
    ).length;
    const visibleFailedObjects =
      visibleRowsInGroup.length - visiblePassedObjects;

    findingGroup.hidden = visibleRowsInGroup.length === 0;
    findingGroup.classList.toggle(
      "finding-group-no-errors",
      visibleRowsInGroup.length > 0 && visibleFailedObjects === 0,
    );
    findingGroup.classList.toggle(
      "finding-group-has-errors",
      visibleFailedObjects > 0,
    );

    const objectPassFailBadge = findingGroup.querySelector(
      "[data-object-pass-fail-count]",
    );
    if (objectPassFailBadge) {
      objectPassFailBadge.innerHTML = renderObjectPassFailCountMarkup(
        visiblePassedObjects,
        visibleFailedObjects,
      );
    }
  });

  const scopeSections = Array.from(
    reportListElement.querySelectorAll(".report-scope-section"),
  );
  scopeSections.forEach((scopeSection) => {
    const hasVisibleGroup =
      scopeSection.querySelector(".finding-group:not([hidden])") !== null;
    const hasStandaloneItem =
      scopeSection.querySelector(".report-item") !== null;

    scopeSection.hidden = !hasVisibleGroup && !hasStandaloneItem;
  });

  const visibleObjectCount = objectRows.filter(
    (objectRow) => !objectRow.hidden,
  ).length;
...
function bindReportFilterControls(reportListElement) {
  ...
  const clearButton = reportListElement.querySelector(
    '[data-report-filter="clear"]',
  );
...
export function renderReportSummary(
  ...
  if (showWarnings) {
    summaryCards.splice(
      4,
      0,
      renderSummaryCard("Warnings", summary.warningCount),
    );
  }
...
export function renderReportFindings(reportListElement, reportData) {
  ...
  const productScopeEnabled = executionProfile?.scopes?.product !== false;
  const systemScopeEnabled = executionProfile?.scopes?.system !== false;
  const productFindings = productScopeEnabled
    ? reportData?.productFindings || []
    : [];
  const systemFindings = systemScopeEnabled
    ? reportData?.systemFindings || []
    : [];
  const hasProductFindings =
    Array.isArray(productFindings) && productFindings.length > 0;
  const hasSystemFindings =
    Array.isArray(systemFindings) && systemFindings.length > 0;
  ...
  if (productScopeEnabled) {
    scopeSections.push(
      renderScopeSection(
        "Tuoteosien tarkastusten loydokset",
        "product",
        productFindings,
        showWarnings,
      ),
  ...
  if (systemScopeEnabled) {
    scopeSections.push(
      renderScopeSection(
        "Jarjestelmien tarkastusten loydokset",
        "system",
        systemFindings,
        showWarnings,
      ),
...
export function getSelectedBcfIssueSelectors() {
  return Array.from(selectedBcfIssueKeys).reduce(
    (selectorAccumulator, selectionKey) => {
