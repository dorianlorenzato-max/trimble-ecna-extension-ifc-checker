...
function setWorkflowStep(activeStep) {
  ...
  if (ui.phaseCaption) {
    ui.phaseCaption.textContent =
      WORKFLOW_STEP_CAPTIONS[activeStep] || WORKFLOW_STEP_CAPTIONS.connect;
  }
}
...
function buildSelectedFileCacheKey(fileDescriptor) {
  ...
  const fileVersionId =
    typeof fileDescriptor.versionId === "string"
      ? fileDescriptor.versionId
      : "";
  const fileName =
    typeof fileDescriptor.name === "string" ? fileDescriptor.name : "";
  const fileLink =
    typeof fileDescriptor.link === "string" ? fileDescriptor.link : "";
...
function updateRunButtonState() {
  const hasRunnableIfc =
    runtimeState.selectedIfcFile &&
    isIfcFileSelection(runtimeState.selectedIfcFile);
...
function normalizeViewerModelAsIfcFile(viewerModel) {
  ...
  const normalizedFileId = pickFirstIdentifier([
    ...
    readIdentifierFromUrlQuery(candidateLink, [
      "fileId",
      "modelId",
      "id",
      "sourceId",
  ...
  return {
    ...
    name:
      pickFirstIdentifier([
        viewerModel?.name,
        viewerModel?.fileName,
        viewerModel?.displayName,
        viewerModel?.title,
      ]) || "Unnamed model",
    link: candidateLink,
    type:
      pickFirstIdentifier([viewerModel?.type, viewerModel?.itemType]) || "FILE",
    fileType:
      pickFirstIdentifier([
...
function renderProjectIfcFileTable(ifcFiles) {
  ...
  ifcFiles.forEach((ifcFile, fileIndex) => {
    ...
    selectInput.checked = isSameIfcFile(
      runtimeState.pickerSelectedIfcFile,
      ifcFile,
    );
    ...
    selectInput.addEventListener("change", () => {
      ...
      renderAppState(
        `Model selected: ${ifcFile.name}. Confirm with OK.`,
        "info",
        "select",
      );
    });
...
async function loadProjectIfcFiles(forceReload = false) {
  const hasCachedProjectFiles =
    !forceReload &&
    runtimeState.projectIfcFilesLoadedProjectId ===
  ...
  runtimeState.projectIfcFilesLoadPromise = (async () => {
    if (!runtimeState.currentProjectId) {
      throw new Error(
        "Current project id was missing, so IFC catalog could not be loaded.",
      );
    }

    const accessToken = await ensureAccessTokenForProjectBrowsing();
    const embeddedViewerApi = await ensureEmbeddedViewer(
      accessToken,
      null,
      null,
    );
    if (
      !embeddedViewerApi?.viewer ||
      typeof embeddedViewerApi.viewer.getModels !== "function"
    ) {
    ...
    const normalizedIfcFiles = viewerModels
      ...
      .filter(
        (normalizedFile) =>
          isIfcFileSelection(normalizedFile) && Boolean(normalizedFile.id),
      )
...
async function ensureAccessTokenForProjectBrowsing() {
  ...
  const permissionResult = await requestAccessTokenPermission(
    runtimeState.workspaceApi,
  );
  const permissionToken = normalizeAccessTokenValue(
    permissionResult?.data || permissionResult,
  );
...
function decodeBase64ToArrayBuffer(base64Payload) {
  ...
  const normalizedBase64 = base64Payload.includes(",")
...
async function extractArrayBufferFromLoadedViewerModel(loadedModelFile) {
  ...
  if (!fileBlobPayload) {
    throw new Error(
      "Viewer returned model metadata without file blob content.",
    );
  }
...
async function waitForLoadedModelBlob(
  ...
  while (Date.now() - startTimestamp < timeoutMilliseconds) {
      ...
      if (typeof viewerApi.viewer.getModels === "function") {
        ...
        if (Array.isArray(availableModels)) {
          ...
          availableModels.forEach((availableModel) => {
            ...
            const availableModelId = normalizeViewerId(
              availableModel.id || availableModel.modelId,
            );
            if (!availableModelId) {
              return;
            }
            ...
            if (
              requestedModelId &&
              comparableModelIdentifiers.includes(requestedModelId)
            ) {
    ...
    for (const modelIdCandidate of modelIdCandidates) {
      try {
        const loadedModel =
          await viewerApi.viewer.getLoadedModel(modelIdCandidate);
        if (loadedModel?.blob) {
          return loadedModel;
        }
      } catch (error) {
  ...
  throw new Error(
    `Embedded viewer did not provide model blob within ${timeoutMilliseconds} ms.`,
  );
}
...
async function ensureEmbeddedViewer(accessToken, modelId, versionId) {
  if (
    runtimeState.embeddedViewerApi &&
    runtimeState.embeddedViewerFrame?.isConnected
  ) {
  ...
  runtimeState.embeddedViewerInitPromise = (async () => {
    ...
    const loadPromise = new Promise((resolve, reject) => {
      hiddenViewerFrame.addEventListener("load", () => resolve(), {
        once: true,
      });
      hiddenViewerFrame.addEventListener(
        "error",
        () => reject(new Error("Hidden viewer iframe failed to load.")),
        {
          once: true,
        },
      );
...
async function downloadIfcViaEmbeddedViewerBridge(selectedFile) {
  ...
  const embeddedViewerApi = await ensureEmbeddedViewer(
    accessToken,
    selectedFile.id,
    selectedFile.versionId || null,
  );
  const loadedViewerModel = await waitForLoadedModelBlob(
    embeddedViewerApi,
    selectedFile.id,
  );
  const modelArrayBuffer =
    await extractArrayBufferFromLoadedViewerModel(loadedViewerModel);
  try {
    await verifyIfcModelReadable(modelArrayBuffer);
  } catch (verificationError) {
    const viewerModelName = String(loadedViewerModel?.name || "").trim();
    const viewerModelType = String(
      loadedViewerModel?.type || loadedViewerModel?.runtimeType || "",
    ).trim();
    const viewerModelIdentity = [viewerModelName, viewerModelType]
      .filter(Boolean)
      .join(" / ");
    const viewerModelSuffix = viewerModelIdentity
      ? ` Viewer payload: ${viewerModelIdentity}.`
      : "";
    throw new Error(
...
async function loadIfcArrayBufferWithFallback(selectedFile, options = {}) {
  ...
  if (canUseViewerBridge) {
    try {
      const viewerBuffer =
        await downloadIfcViaEmbeddedViewerBridge(selectedFile);
      return {
      ...
      if (!allowDirectFallback) {
        throw new Error(
          `Viewer bridge failed: ${
            viewerBridgeError instanceof Error
              ? viewerBridgeError.message
              : String(viewerBridgeError)
          }`,
        );
      }

      try {
        const directBuffer = await downloadSelectedIfcAsArrayBuffer(
          selectedFile,
          accessToken,
          runtimeState.currentProjectId,
        );
  ...
  if (!allowDirectFallback) {
    throw new Error(
      "Direct download fallback is disabled and selected model did not contain viewer-compatible id.",
    );
  }
...
async function openProjectIfcPicker() {
  ...
  try {
    if (!runtimeState.currentProjectId) {
      throw new Error(
        "Current project was not available. Refresh the view and try again.",
      );
    }

    const projectIfcFiles = await loadProjectIfcFiles(false);
    const matchingSelectedFile = projectIfcFiles.find((ifcFile) =>
      isSameIfcFile(ifcFile, runtimeState.selectedIfcFile),
    );
...
async function confirmPickerSelection() {
  ...
  if (!stagedSelection) {
    renderAppState(
      "Select an IFC model before confirming.",
      "warning",
      "select",
    );
    return;
  ...
  const sameFileAsCurrentSelection =
    ...
    ((stagedSelection.id && currentSelectedFile.id === stagedSelection.id) ||
      (!stagedSelection.id &&
        currentSelectedFile.name === stagedSelection.name));

  const committedSelection = {
    ...stagedSelection,
    link:
      stagedSelection.link ||
      (sameFileAsCurrentSelection ? currentSelectedFile.link : ""),
    source:
      stagedSelection.source ||
      (sameFileAsCurrentSelection
        ? currentSelectedFile.source
        : "Modal / IFC table"),
    versionId:
      stagedSelection.versionId ||
      (sameFileAsCurrentSelection ? currentSelectedFile.versionId : null),
  };
  ...
  try {
    ...
    renderAppState(
      `Loading selected model into browser memory: ${committedSelection.name}`,
      "info",
      "select",
    );
    const loadedModel = await loadIfcArrayBufferWithFallback(
      committedSelection,
      {
    ...
    renderSelectedFile(
      ui.selectedFileName,
      ui.selectedFileSource,
      committedSelection.name,
      committedSelection.source,
    );
    const readyMessage =
...
async function runSelectedIfcCheck() {
  if (
    ...
    renderAppState(
      "Select an IFC model before running validation.",
      "warning",
      "select",
    );
    return;
  ...
  try {
    ...
    const selectedFileKey = buildSelectedFileCacheKey(
      runtimeState.selectedIfcFile,
    );
    ...
    if (
      cachedModel &&
      cachedModel.fileKey === selectedFileKey &&
      cachedModel.arrayBuffer
    ) {
      ifcFileBuffer = cachedModel.arrayBuffer;
      renderAppState(
        `Using preloaded model: ${runtimeState.selectedIfcFile.name}`,
        "info",
        "process",
      );
    } else {
      renderAppState(
        `Loading model: ${runtimeState.selectedIfcFile.name}`,
        "info",
        "process",
      );
    ...
    const checkReport = runModelChecks(
      parsedModel,
      runtimeState.originalRuleDataset,
    );
    ...
    renderAppState(
      completionMessage,
      checkReport.summary.totalIssues > 0 ? "warning" : "success",
      "report",
    );
  } catch (error) {
    renderAppState(
...
async function handleWorkspaceEvent(eventName, args) {
  if (eventName === WORKSPACE_EVENT_ACCESS_TOKEN) {
    ...
    if (runtimeState.embeddedViewerApi && runtimeState.accessToken) {
      runtimeState.embeddedViewerApi.embed
        .setTokens({ accessToken: runtimeState.accessToken })
        .catch(() => {
    ...
    if (!runtimeState.selectedIfcFile) {
      renderAppState(
        "Connection ready. Select model for validation.",
        "success",
        "select",
      );
    }
  ...
  if (
    eventName === WORKSPACE_EVENT_FILE_SELECTED ||
    eventName === WORKSPACE_EVENT_FILE_VIEW_CLICKED
  ) {
    const selectedFile = normalizeSelectedFile(args);
    if (!selectedFile) {
      renderAppState(
        "No file metadata was found in selected event payload.",
        "warning",
        "select",
      );
      return;
    ...
    renderSelectedFile(
      ...
      ui.selectedFileSource,
      selectedFile.name,
      selectedFile.source || eventName,
    );

    if (!isIfcFileSelection(selectedFile)) {
      renderAppState(
        "Selected file is not IFC. Choose a .ifc file.",
        "warning",
        "select",
      );
    } else {
      renderAppState(
        `Selected model: ${selectedFile.name}`,
        "success",
        "select",
      );
    }
  ...
  if (
    ...
    args?.data === "main_clicked"
  ) {
    renderAppState(
      "Application activated. Select model and run validation.",
      "info",
      "select",
    );
  }
...
async function initializeApplication() {
  ...
  await setExtensionStatusMessage(
    runtimeState.workspaceApi,
    "IFC Checker extension active",
  );
  ...
  renderAppState(
    `Connected to project: ${runtimeState.currentProjectName}`,
    "success",
    "select",
  );
  ...
  prefetchProjectIfcFilesInBackground();
  renderAppState(
    "Ready. Select model and run validation.",
    "success",
    "select",
  );
...
document.addEventListener("keydown", (event) => {
  if (
    event.key === "Escape" &&
    ui.ifcPickerModal &&
    !ui.ifcPickerModal.classList.contains("hidden")
  ) {
    closeIfcPickerModal();
  }
});
