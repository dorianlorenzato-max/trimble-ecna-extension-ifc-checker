...
const TOPICS_API_MEDIA_TYPE_CONNECT =
  "application/vnd.trimble.connect.app+json";
const TOPICS_API_MEDIA_TYPE_CONNECT_LEGACY =
  "application/vnd.trimble.connect.app";
...
const ui = {
  ...
  editCheckSettingsButton: document.getElementById(
    "edit-check-settings-button",
  ),
  ...
  checkSelectionCloseButton: document.getElementById(
    "check-selection-close-button",
  ),
  checkSelectionCancelButton: document.getElementById(
    "check-selection-cancel-button",
  ),
  checkSelectionConfirmButton: document.getElementById(
    "check-selection-confirm-button",
  ),
  checkSelectionEnableAllButton: document.getElementById(
    "check-selection-enable-all-button",
  ),
  checkSelectionDisableAllButton: document.getElementById(
    "check-selection-disable-all-button",
  ),
  checkSelectionDefaultsButton: document.getElementById(
    "check-selection-defaults-button",
  ),
...
function cloneCheckExecutionOptions(checkExecutionOptions) {
  const sourceOptions =
    checkExecutionOptions && typeof checkExecutionOptions === "object"
...
function normalizePropertyStateValue(stateValue) {
  return String(stateValue || "").toLowerCase() === "unchecked"
    ? "unchecked"
    : "checked";
}
  ...
  const buildGroupedMap = (definitions) => {
    ...
    Array.from(groupedMap.keys()).forEach((groupName) => {
      ...
      groupDefinitions.sort((leftDefinition, rightDefinition) =>
        String(leftDefinition.label).localeCompare(
          String(rightDefinition.label),
          "fi",
          { sensitivity: "base" },
        ),
      );
    ...
    return new Map(
      Array.from(groupedMap.entries()).sort((leftEntry, rightEntry) =>
        String(leftEntry[0]).localeCompare(String(rightEntry[0]), "fi", {
          sensitivity: "base",
        }),
      ),
    );
...
function buildDefaultCheckExecutionOptions(ruleDataset) {
  ...
  allDefinitions.forEach((definition) => {
    ...
    propertyStates[definition.group][definition.label] =
      normalizePropertyStateValue(defaultState);
  });
...
None
*/
function mergeCheckExecutionOptionsWithDefaults(
  defaultExecutionOptions,
  existingExecutionOptions,
) {
  const mergedOptions = cloneCheckExecutionOptions(defaultExecutionOptions);
  const sourceOptions =
    existingExecutionOptions && typeof existingExecutionOptions === "object"
      ? existingExecutionOptions
      : {};

  mergedOptions.enabledScopes.product =
    sourceOptions?.enabledScopes?.product !== false;
  mergedOptions.enabledScopes.system =
    sourceOptions?.enabledScopes?.system !== false;

  mergedOptions.enabledCheckPhases.productIdentification =
    sourceOptions?.enabledCheckPhases?.productIdentification !== false;
  mergedOptions.enabledCheckPhases.productContent =
    sourceOptions?.enabledCheckPhases?.productContent !== false;
  mergedOptions.enabledCheckPhases.systemIdentification =
    sourceOptions?.enabledCheckPhases?.systemIdentification !== false;
  mergedOptions.enabledCheckPhases.systemContent =
    sourceOptions?.enabledCheckPhases?.systemContent !== false;
  mergedOptions.showWarnings = sourceOptions?.showWarnings === true;

  Object.keys(mergedOptions.propertyStates).forEach((groupName) => {
    Object.keys(mergedOptions.propertyStates[groupName]).forEach(
      (propertyLabel) => {
        const existingState =
          sourceOptions?.propertyStates?.[groupName]?.[propertyLabel];
        if (existingState !== undefined) {
          mergedOptions.propertyStates[groupName][propertyLabel] =
            normalizePropertyStateValue(existingState);
        }
      },
    );
  });
...
function ensureCheckExecutionOptionsInitialized(ruleDataset) {
  const defaultExecutionOptions =
    buildDefaultCheckExecutionOptions(ruleDataset);
...
function countScopeSelectedProperties(scopeGroupedDefinitions, propertyStates) {
  ...
  Array.from(scopeGroupedDefinitions.values()).forEach((groupDefinitions) => {
    groupDefinitions.forEach((definition) => {
      totalCount += 1;
      const stateValue = normalizePropertyStateValue(
        propertyStates?.[definition.group]?.[definition.label],
      );
...
function summarizeCheckExecutionOptions(ruleDataset, checkExecutionOptions) {
  const groupedDefinitionsByScope =
    buildScopeGroupedPropertyDefinitions(ruleDataset);
  const productScopeEnabled =
    checkExecutionOptions?.enabledScopes?.product !== false;
  const systemScopeEnabled =
    checkExecutionOptions?.enabledScopes?.system !== false;
  const productIdentificationEnabled =
    checkExecutionOptions?.enabledCheckPhases?.productIdentification !== false;
  const productContentEnabled =
    checkExecutionOptions?.enabledCheckPhases?.productContent !== false;
  const systemIdentificationEnabled =
    checkExecutionOptions?.enabledCheckPhases?.systemIdentification !== false;
  const systemContentEnabled =
    checkExecutionOptions?.enabledCheckPhases?.systemContent !== false;
  const showWarningsEnabled = checkExecutionOptions?.showWarnings === true;

  const productCounts = countScopeSelectedProperties(
    groupedDefinitionsByScope.product,
    checkExecutionOptions?.propertyStates || {},
  );
  const systemCounts = countScopeSelectedProperties(
...
function renderCheckSelectionScopeSection(
  ...
  const scopeEnabled =
    draftExecutionOptions?.enabledScopes?.[scopeKey] !== false;
  const identificationPhaseKey = `${scopeKey}Identification`;
  const contentPhaseKey = `${scopeKey}Content`;
  const isIdentificationEnabled =
    draftExecutionOptions?.enabledCheckPhases?.[identificationPhaseKey] !==
    false;
  const isContentEnabled =
    draftExecutionOptions?.enabledCheckPhases?.[contentPhaseKey] !== false;
  const scopeSelectionCounts = countScopeSelectedProperties(
    groupedDefinitions,
    draftExecutionOptions?.propertyStates || {},
  );
...
None
*/
function renderCheckSelectionModalContent(
  groupedDefinitionsByScope,
  draftExecutionOptions,
) {
...
*/
function applyGlobalCheckSelectionState(
  draftExecutionOptions,
  groupedDefinitionsByScope,
  checkedState,
  enabledScopeState,
  ...
  draftExecutionOptions.enabledCheckPhases.productIdentification =
    Boolean(enabledScopeState);
  draftExecutionOptions.enabledCheckPhases.productContent =
    Boolean(enabledScopeState);
  draftExecutionOptions.enabledCheckPhases.systemIdentification =
    Boolean(enabledScopeState);
  draftExecutionOptions.enabledCheckPhases.systemContent =
    Boolean(enabledScopeState);
...
function openCheckSelectionModal(ruleDataset) {
  return new Promise((resolvePromise) => {
    ...
    if (
      !ui.checkSelectionModal ||
      !ui.checkSelectionDialog ||
      !ui.checkSelectionContent
    ) {
      resolvePromise(
        cloneCheckExecutionOptions(runtimeState.checkExecutionOptions),
      );
      return;
    }
    ...
    const groupedDefinitionsByScope =
      buildScopeGroupedPropertyDefinitions(ruleDataset);
    let draftExecutionOptions = cloneCheckExecutionOptions(
      runtimeState.checkExecutionOptions,
    );

    const renderModalContent = () => {
      ui.checkSelectionContent.innerHTML = renderCheckSelectionModalContent(
        groupedDefinitionsByScope,
        draftExecutionOptions,
      );
    };
    ...
    const cleanupAndResolve = (resolvedValue) => {
      ...
      ui.checkSelectionContent.removeEventListener(
        "change",
        handleContentChange,
      );
      ...
      ui.checkSelectionConfirmButton?.removeEventListener(
        "click",
        handleConfirm,
      );
      ui.checkSelectionEnableAllButton?.removeEventListener(
        "click",
        handleEnableAll,
      );
      ui.checkSelectionDisableAllButton?.removeEventListener(
        "click",
        handleDisableAll,
      );
      ui.checkSelectionDefaultsButton?.removeEventListener(
        "click",
        handleDefaults,
      );
    ...
    const handleConfirm = () => {
      const isProductScopeEnabled =
        draftExecutionOptions?.enabledScopes?.product !== false;
      const isSystemScopeEnabled =
        draftExecutionOptions?.enabledScopes?.system !== false;
      ...
      if (!hasProductChecksSelected && !hasSystemChecksSelected) {
        setStatus(
          "Valitse ainakin yksi tarkastus ennen suoritusta.",
          "warning",
        );
        return;
      }
    ...
    const handleEnableAll = () => {
      applyGlobalCheckSelectionState(
        draftExecutionOptions,
        groupedDefinitionsByScope,
        "checked",
        true,
    ...
    const handleContentChange = (changeEvent) => {
      ...
      const scopeKeyFromToggle = eventTarget.getAttribute(
        "data-check-scope-enabled",
      );
      if (scopeKeyFromToggle) {
        draftExecutionOptions.enabledScopes[scopeKeyFromToggle] =
          eventTarget.checked;
        renderModalContent();
        return;
      }

      const phaseKeyFromToggle = eventTarget.getAttribute(
        "data-check-phase-enabled",
      );
      if (phaseKeyFromToggle) {
        draftExecutionOptions.enabledCheckPhases[phaseKeyFromToggle] =
          eventTarget.checked;
        renderModalContent();
        return;
      }
      ...
      if (eventTarget.getAttribute("data-check-property") === "true") {
        const propertyGroup = decodeDataAttributeValue(
          eventTarget.getAttribute("data-check-group"),
        );
        const propertyLabel = decodeDataAttributeValue(
          eventTarget.getAttribute("data-check-label"),
        );
        ...
        draftExecutionOptions.propertyStates[propertyGroup][propertyLabel] =
    ...
    ui.checkSelectionEnableAllButton?.addEventListener(
      "click",
      handleEnableAll,
    );
    ui.checkSelectionDisableAllButton?.addEventListener(
      "click",
      handleDisableAll,
    );
...
function updateReportExportActionButtonState() {
  const hasCheckResult = Boolean(
    runtimeState.latestCheckResult && runtimeState.downloadedIfcFile,
  );
...
function syncFileTableSelectionState() {
  ...
  const tableRows = Array.from(
    ui.fileTableBody.querySelectorAll("tr[data-row-key]"),
  );
  tableRows.forEach((tableRow) => {
    const rowKey = String(tableRow.getAttribute("data-row-key") || "");
    const isSelectedRow =
      rowKey !== "" && rowKey === runtimeState.selectedFileRowKey;
    tableRow.classList.toggle("is-selected", isSelectedRow);

    const rowRadioInput = tableRow.querySelector(
      'input[type="radio"][name="ifc-file-selection"]',
    );
...
async function registerMainMenuIfSupported(workspaceApi) {
  ...
  const setMenuFunction =
    workspaceApi?.ui && typeof workspaceApi.ui.setMenu === "function"
      ? workspaceApi.ui.setMenu.bind(workspaceApi.ui)
      : null;
...
async function connectWorkspaceApi() {
  ...
  if (
    ...
    typeof window.TrimbleConnectWorkspace.connect !== "function"
  ) {
    throw new Error(
      "Trimble Workspace API script was not available on the page.",
    );
  }

  runtimeState.workspaceApi = await window.TrimbleConnectWorkspace.connect(
    window.parent,
...
async function readCurrentProjectContext(workspaceApi) {
  ...
  if (!getCurrentProjectFunction) {
    throw new Error(
      "Workspace project API did not expose project read method.",
    );
  }

  const rawProject = await getCurrentProjectFunction();
  const projectId = pickFirstNonEmptyString([
    rawProject?.id,
    rawProject?.projectId,
  ]);
  const projectName = pickFirstNonEmptyString([
    rawProject?.name,
  ...
  if (!isNonEmptyString(projectId)) {
    throw new Error(
      "Current project id was missing from Workspace API response.",
    );
  }
...
async function ensureAccessToken(workspaceApi) {
  ...
  if (
    !workspaceApi?.extension ||
    typeof workspaceApi.extension.requestPermission !== "function"
  ) {
...
function extractFolderItems(responsePayload) {
  ...
  const firstArrayCandidate = candidateCollections.find((candidateValue) =>
...
async function loadFolderItems(coreApiBaseUrl, folderId, accessToken) {
  ...
  if (!folderResponse.ok) {
    throw new Error(
      `Folder listing request failed (${folderResponse.status}) at ${coreApiBaseUrl}.`,
    );
  }
...
async function loadJsonFromCoreApi(requestUrl, accessToken) {
  ...
  if (!response.ok) {
    throw new Error(
      `Core API request failed (${response.status}) for ${requestUrl}.`,
    );
  }
...
async function resolveProjectRootFolderId(
  ...
  try {
    ...
    const projectPayload = await loadJsonFromCoreApi(
      projectEndpoint,
      accessToken,
    );
...
function isFolderEntry(rawItem) {
  ...
  if (
    normalizedTypeText.includes("folder") ||
    normalizedTypeText.includes("directory")
  ) {
    return true;
  }
...
*/
async function collectFilesFromProjectTree(
  coreApiBaseUrl,
  accessToken,
  rootFolderId,
) {
  ...
  while (folderQueue.length > 0) {
    ...
    const folderItems = await loadFolderItems(
      coreApiBaseUrl,
...
async function loadProjectFiles(accessToken, projectId, preferredRootFolderId) {
  ...
  for (const coreApiBaseUrl of coreApiBaseUrlCandidates) {
    try {
      ...
      const collectedFiles = await collectFilesFromProjectTree(
        coreApiBaseUrl,
        accessToken,
        resolvedRootFolderId,
      );
      return {
        fileEntries: collectedFiles.filter((fileEntry) =>
          isIfcFileName(fileEntry.name),
        ),
        coreApiBaseUrl,
      };
    } catch (error) {
  ...
  throw new Error(
    `Project file listing failed for all API hosts. ${failedAttempts.join(" | ")}`,
  );
}
...
*/
async function requestSignedDownloadUrl(
  coreApiBaseUrl,
  accessToken,
  fileId,
  versionId,
  ...
  const responsePayload = await loadJsonFromCoreApi(
    downloadUrlEndpoint,
    accessToken,
  );
  const signedDownloadUrl = pickFirstNonEmptyString([
    responsePayload?.url,
  ...
  if (!isNonEmptyString(signedDownloadUrl)) {
    throw new Error(
      "Download URL endpoint did not return a usable signed URL.",
    );
  }
...
*/
async function downloadIfcToBrowserMemory(
  signedDownloadUrl,
  selectedFileEntry,
) {
  ...
  if (!downloadResponse.ok) {
    throw new Error(
      `Signed file download failed with status ${downloadResponse.status}.`,
    );
  }
...
async function runModelCheckFromDownloadedFile(downloadedIfcFile) {
  if (
    !downloadedIfcFile ||
    !(downloadedIfcFile.buffer instanceof ArrayBuffer)
  ) {
    setStatus(
      "Downloaded IFC payload was missing, so check could not start.",
      "error",
    );
    return;
  }
  ...
  try {
    ...
    setStatus(
      `Preparing checker runtime for ${downloadedIfcFile.name}...`,
      "info",
    );
    ...
    const parsedModel = await parseIfcModelForChecking(
      downloadedIfcFile.buffer,
    ...
    const showWarningsInResultView =
      checkResult?.executionProfile?.display?.showWarnings !== false;
    ...
    if (
      runtimeState.workspaceApi?.extension &&
      typeof runtimeState.workspaceApi.extension.setStatusMessage === "function"
    ) {
      await runtimeState.workspaceApi.extension.setStatusMessage(
        successMessage,
      );
    }
  } catch (error) {
...
async function clearLoadedModelAndResetWorkflow() {
  if (
    ...
    setStatus(
      "Wait until current operation finishes before clearing model.",
      "warning",
    );
    return;
  }
  ...
  if (
    !Array.isArray(runtimeState.latestIfcFileEntries) ||
    runtimeState.latestIfcFileEntries.length === 0
  ) {
    await loadAndRenderProjectFiles();
  } else {
    setStatus(
      "Loaded model was cleared from memory. Select file and run download again.",
      "info",
    );
    ...
    if (
      ...
      await runtimeState.workspaceApi.extension.setStatusMessage(
        "Loaded model cleared from memory.",
      );
    }
  }
...
async function editCheckSettingsForLoadedModel() {
  if (
    ...
    setStatus(
      "Wait until current operation finishes before editing check settings.",
      "warning",
    );
    return;
  }
  ...
  if (
    !runtimeState.downloadedIfcFile ||
    !(runtimeState.downloadedIfcFile.buffer instanceof ArrayBuffer)
  ) {
    setStatus(
      "Load one IFC model to memory before editing check settings.",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    const selectedExecutionOptions = await openCheckSelectionModal(
      runtimeState.originalRuleDataset,
    );
    ...
    const selectionSummary = summarizeCheckExecutionOptions(
      runtimeState.originalRuleDataset,
      runtimeState.checkExecutionOptions,
    );
...
function selectPreferredExtensionOptionValue(optionValues, preferredKeywords) {
  ...
  const normalizedKeywords = collectUniqueTrimmedStringValues(
    preferredKeywords,
  ).map((keyword) => keyword.toLowerCase());

  for (const preferredKeyword of normalizedKeywords) {
    const matchedOption = normalizedOptions.find((optionValue) =>
      optionValue.toLowerCase().includes(preferredKeyword),
    );
    if (matchedOption) {
...
None
*/
function findExtensionOptionValueByCaseInsensitiveExactMatch(
  optionValues,
  targetValue,
) {
  const normalizedOptions = collectUniqueTrimmedStringValues(optionValues);
  ...
  return (
    normalizedOptions.find(
      (optionValue) => optionValue.toLowerCase() === normalizedTargetValue,
    ) || ""
  );
}
...
function buildBcfProjectExtensionOptions(extensionPayload) {
  const topicTypes = collectUniqueTrimmedStringValues(
    extensionPayload?.topic_type,
  );
  const topicStatuses = collectUniqueTrimmedStringValues(
    extensionPayload?.topic_status,
  );
  const priorities = collectUniqueTrimmedStringValues(
    extensionPayload?.priority,
  );
  const stages = collectUniqueTrimmedStringValues(extensionPayload?.stage);
  const topicLabels = collectUniqueTrimmedStringValues(
    extensionPayload?.topic_label,
  );
  const projectActions = collectUniqueTrimmedStringValues(
    extensionPayload?.project_actions,
  );
  const topicActions = collectUniqueTrimmedStringValues(
    extensionPayload?.topic_actions,
  );

  const defaultTopicType = selectPreferredExtensionOptionValue(topicTypes, [
    "error",
    "warning",
    "issue",
    "clash",
  ...
  const defaultTopicStatus = selectPreferredExtensionOptionValue(
    topicStatuses,
    ["open", "reopened", "in progress"],
  );
  const defaultPriority = selectPreferredExtensionOptionValue(priorities, [
    "high",
    "medium",
    "low",
  ]);
  const requiredTopicLabel =
    findExtensionOptionValueByCaseInsensitiveExactMatch(
      topicLabels,
      IFC_CHECKER_TOPIC_LABEL,
    );
  const defaultTopicLabel =
    requiredTopicLabel ||
    selectPreferredExtensionOptionValue(topicLabels, ["issue"]);
...
None
*/
function normalizeBcfLengthUnitToMeters(
  lengthUnitToMetersCandidate,
  fallbackValue = DEFAULT_BCF_LENGTH_UNIT_TO_METERS,
) {
  const normalizedFallbackValue = Number(fallbackValue);
  const normalizedCandidateValue = Number(lengthUnitToMetersCandidate);

  if (
    Number.isFinite(normalizedCandidateValue) &&
    normalizedCandidateValue > 0
  ) {
...
function resolveBcfLengthUnitToMetersFromCheckResult(latestCheckResult) {
  return normalizeBcfLengthUnitToMeters(
    latestCheckResult?.modelContext?.ifcLengthUnitToMeters,
  );
}
...
None
*/
function convertPlacementPointToBcfMeters(
  placementPointCandidate,
  lengthUnitToMetersCandidate,
) {
  ...
  const lengthUnitToMeters = normalizeBcfLengthUnitToMeters(
    lengthUnitToMetersCandidate,
  );
  return {
...
function buildDefaultTopicTypeForIssue(issueSeverity, extensionOptions) {
  const normalizedSeverity = String(issueSeverity || "")
    .trim()
    .toLowerCase();
  if (normalizedSeverity === "error") {
    const errorType = selectPreferredExtensionOptionValue(
      extensionOptions?.topicTypes || [],
      ["error", "issue", "clash"],
    );
    return errorType || extensionOptions?.defaultTopicType || "Error";
  }

  const warningType = selectPreferredExtensionOptionValue(
    extensionOptions?.topicTypes || [],
    ["warning", "information", "issue"],
  );
...
None
*/
function buildSelectOptionMarkup(
  optionValues,
  selectedValue,
  ...
  if (
    ...
    !normalizedOptions.includes(normalizedSelectedValue)
  ) {
  ...
  const optionMarkup = normalizedOptions
    .map((optionValue) => {
      const selectedAttribute =
        optionValue === normalizedSelectedValue ? " selected" : "";
  ...
  const emptySelectedAttribute =
    normalizedSelectedValue === "" ? " selected" : "";
...
function renderBcfTopicCreationModalContent(issueRows, extensionOptions) {
  ...
  const defaultStatusValue =
    normalizedExtensionOptions.defaultTopicStatus || "Open";
  const defaultPriorityValue =
    normalizedExtensionOptions.defaultPriority || "Medium";
  const defaultLabels = collectUniqueTrimmedStringValues([
    normalizedExtensionOptions.defaultTopicLabel,
  ]);
  ...
  const topicRowsMarkup = normalizedIssueRows
    .map((issueRow, issueIndex) => {
      const normalizedPlacementPoint = normalizeBcfPlacementPoint(
        issueRow?.placementPoint,
      );
      const defaultTopicType = buildDefaultTopicTypeForIssue(
        issueRow?.severity,
        normalizedExtensionOptions,
      );
      const topicTypeOptions = collectUniqueTrimmedStringValues([
        defaultTopicType,
        ...(normalizedExtensionOptions.topicTypes || []),
      ]);
      const defaultTitle = buildDefaultBcfTopicTitle(issueRow);
      const defaultDescription = String(issueRow?.message || "").trim();
      const severityLabel =
        String(issueRow?.severity || "warning").toLowerCase() === "error"
          ? "Error"
          : "Warning";
...
function openBcfTopicCreationModal(issueRows, extensionOptions) {
  return new Promise((resolvePromise) => {
    ...
    const renderModalContent = () => {
      ...
      ui.bcfCreateContent.innerHTML = renderBcfTopicCreationModalContent(
        normalizedIssueRows,
        normalizedExtensionOptions,
      );
    };
    ...
    const handleContentChange = (changeEvent) => {
      ...
      if (
        eventTarget.getAttribute("data-bcf-generic-field") !==
        "create-viewpoints"
      ) {
        return;
      }

      const cameraToggle = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="use-placement-camera"]',
      );
    ...
    const handleConfirm = () => {
      const genericStatusSelect = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="topic-status"]',
      );
      const genericPrioritySelect = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="priority"]',
      );
      const genericStageSelect = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="stage"]',
      );
      const genericDueDateInput = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="due-date"]',
      );
      const genericReferenceLinksInput = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="reference-links"]',
      );
      const createViewpointsToggle = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="create-viewpoints"]',
      );
      const usePlacementCameraToggle = ui.bcfCreateContent.querySelector(
        '[data-bcf-generic-field="use-placement-camera"]',
      );
      const allowedProjectLabels = collectUniqueTrimmedStringValues(
        normalizedExtensionOptions.topicLabels || [],
      );
      const requiredProjectLabel =
        findExtensionOptionValueByCaseInsensitiveExactMatch(
          allowedProjectLabels,
          normalizedExtensionOptions.requiredTopicLabel ||
            IFC_CHECKER_TOPIC_LABEL,
        );

      const selectedProjectLabels = Array.from(
        ui.bcfCreateContent.querySelectorAll(
          '[data-bcf-generic-label-option="true"]:checked',
        ),
      )
        .map((selectedLabelElement) =>
          selectedLabelElement instanceof HTMLInputElement
            ? String(selectedLabelElement.value || "").trim()
            : "",
        )
        .filter(
          (selectedLabelValue) =>
            selectedLabelValue !== "" &&
            (allowedProjectLabels.length === 0 ||
              allowedProjectLabels.includes(selectedLabelValue)),
        );
      const requiredLabels = requiredProjectLabel ? [requiredProjectLabel] : [];

      const selectedTopics = [];
      const topicRowElements = Array.from(
        ui.bcfCreateContent.querySelectorAll("[data-bcf-topic-row-index]"),
      );
      topicRowElements.forEach((topicRowElement) => {
        const issueRowIndex = Number(
          topicRowElement.getAttribute("data-bcf-topic-row-index") || "-1",
        );
        if (
          !Number.isInteger(issueRowIndex) ||
          issueRowIndex < 0 ||
          issueRowIndex >= normalizedIssueRows.length
        ) {
          return;
        }

        const sourceIssueRow = normalizedIssueRows[issueRowIndex];
        const includeToggle = topicRowElement.querySelector(
          '[data-bcf-topic-field="include"]',
        );
        const includeTopic =
          includeToggle instanceof HTMLInputElement
        ...
        const topicTypeSelect = topicRowElement.querySelector(
          '[data-bcf-topic-field="topic-type"]',
        );
        const titleInput = topicRowElement.querySelector(
          '[data-bcf-topic-field="title"]',
        );
        const descriptionInput = topicRowElement.querySelector(
          '[data-bcf-topic-field="description"]',
        );
        ...
        selectedTopics.push({
          ...
          placementPoint: normalizeBcfPlacementPoint(
            sourceIssueRow?.placementPoint,
          ),
        });
      });
      ...
      if (selectedTopics.length === 0) {
        setInlineMessage(
          "Select at least one row with non-empty title and description before creating BCF topics.",
        );
        return;
      }
      ...
      cleanupAndResolve({
        generic: {
          ...
          labels: collectUniqueTrimmedStringValues([
            ...selectedProjectLabels,
            ...requiredLabels,
          ]),
          referenceLinks: parseDelimitedTextValues(
            genericReferenceLinksInput?.value || "",
          ),
          createViewpoints:
            createViewpointsToggle instanceof HTMLInputElement
              ? createViewpointsToggle.checked
              : false,
          usePlacementCamera:
            usePlacementCameraToggle instanceof HTMLInputElement
              ? usePlacementCameraToggle.checked
              : false,
        },
...
async function requestTopicsApiJson(
  ...
  try {
    ...
    const networkError = new Error(
      `Topics API request failed before response: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`,
    );
  ...
  if (!response.ok) {
    ...
    const requestError = new Error(
      parsedErrorMessage
        ? `${fallbackMessage} ${parsedErrorMessage}`
        : fallbackMessage,
    );
...
function deriveTopicsHostFromTrimbleHost(trimbleHostName) {
  const normalizedHost = String(trimbleHostName || "")
    .trim()
    .toLowerCase();
  if (!normalizedHost) {
    return "";
  ...
  const appLikeHost = normalizedHost.startsWith("web.")
    ? normalizedHost.replace(/^web\./, "app.")
    : normalizedHost;
  const stageMatch = appLikeHost.match(
    /^app(\d*)\.stage\.connect\.trimble\.com$/,
  );
  if (stageMatch) {
  ...
  const productionMatch = appLikeHost.match(
    /^app(\d+)\.connect\.trimble\.com$/,
  );
  if (productionMatch) {
...
async function resolveTopicsApiBaseUrlForProject(accessToken, projectId) {
  ...
  for (const topicsApiBaseUrl of topicsApiBaseUrlCandidates) {
    ...
    try {
      ...
      if (statusCode === 401 || statusCode === 403) {
        throw new Error(
          `Topics API authorization failed at ${topicsApiBaseUrl}. ${errorMessage}`,
        );
      }
  ...
  throw new Error(
    `Could not resolve Topics API host for current project. ${failedAttemptText}`,
  );
}
...
*/
async function fetchProjectBcfExtensionsPayload(
  topicsApiBaseUrl,
  accessToken,
  projectId,
) {
  ...
  try {
    extensionsPayload = await requestTopicsApiJson(
      extensionsEndpoint,
      accessToken,
      {
        method: "GET",
...
*/
async function updateProjectBcfExtensions(
  topicsApiBaseUrl,
  accessToken,
  projectId,
...
async function ensureProjectTopicLabelExists(
  ...
  const currentTopicLabels = collectUniqueTrimmedStringValues(
    normalizedExtensionsPayload?.topic_label,
  );
  const existingRequiredLabel =
    findExtensionOptionValueByCaseInsensitiveExactMatch(
      currentTopicLabels,
      normalizedRequiredLabel,
    );
  if (existingRequiredLabel) {
    return normalizedExtensionsPayload;
  }

  const updatedTopicLabels = collectUniqueTrimmedStringValues([
    ...currentTopicLabels,
    normalizedRequiredLabel,
  ]);
  try {
    ...
    const updatedLabelValues = collectUniqueTrimmedStringValues(
      updatedExtensionsPayload?.topic_label,
    );
    if (
      findExtensionOptionValueByCaseInsensitiveExactMatch(
        updatedLabelValues,
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(
      `Could not add required topic label "${normalizedRequiredLabel}" to project extensions. ${errorMessage}`,
    );
  }

  const refreshedExtensionsPayload = await fetchProjectBcfExtensionsPayload(
    topicsApiBaseUrl,
    accessToken,
    projectId,
  );
  const refreshedLabels = collectUniqueTrimmedStringValues(
    refreshedExtensionsPayload?.topic_label,
  );
  if (
    findExtensionOptionValueByCaseInsensitiveExactMatch(
      refreshedLabels,
  ...
  throw new Error(
    `Could not confirm required topic label "${normalizedRequiredLabel}" in project extensions.`,
  );
}
...
*/
async function loadProjectBcfExtensionOptions(
  topicsApiBaseUrl,
  accessToken,
  projectId,
) {
  const extensionsPayload = await fetchProjectBcfExtensionsPayload(
    topicsApiBaseUrl,
    accessToken,
    projectId,
  );
...
- Executes one or more Topics API requests.
*/
async function loadProjectBcfFileInformationEntries(
  topicsApiBaseUrl,
  accessToken,
  projectId,
) {
  ...
  while (isNonEmptyString(nextRequestUrl)) {
    ...
    if (loadedPageCount > maximumPageCount) {
      throw new Error(
        `Topics API file-information pagination exceeded ${maximumPageCount} pages.`,
      );
    }

    const responseWithMetadata = await requestTopicsApiJson(
      nextRequestUrl,
      accessToken,
      {
        method: "GET",
    ...
    const pagePayload = Array.isArray(responseWithMetadata?.payload)
      ? responseWithMetadata.payload
      : [];
...
function normalizeTopicFileReferenceForBcfUpdate(fileReferenceCandidate) {
  ...
  const normalizedIfcProject = String(
    fileReferenceCandidate.ifc_project || "",
  ).trim();
  const normalizedIfcSpatialStructureElement = String(
    fileReferenceCandidate.ifc_spatial_structure_element || "",
  ).trim();
  const normalizedFileName = String(
    fileReferenceCandidate.filename || "",
  ).trim();
  const normalizedDate = String(fileReferenceCandidate.date || "").trim();
  const normalizedReference = String(
    fileReferenceCandidate.reference || "",
  ).trim();
  ...
  if (normalizedIfcSpatialStructureElement) {
    normalizedTopicFileReference.ifc_spatial_structure_element =
      normalizedIfcSpatialStructureElement;
  }
  if (normalizedDate) {
...
None
*/
function resolveTopicFileReferenceForSelectedModel(
  projectFileInformationEntries,
  selectedModelFile,
) {
  const selectedModelFileId = String(selectedModelFile?.fileId || "").trim();
  const selectedModelVersionId = String(
    selectedModelFile?.versionId || "",
  ).trim();
  const selectedModelFileName = String(selectedModelFile?.name || "").trim();
  if (!selectedModelFileId || !selectedModelFileName) {
    throw new Error(
      "Selected IFC file id and file name are required for Topics file linking.",
    );
  }

  const normalizedCandidateReferences = (
    Array.isArray(projectFileInformationEntries)
      ? projectFileInformationEntries
      : []
  )
    .map((projectFileInformationEntry) =>
      normalizeTopicFileReferenceForBcfUpdate(
        projectFileInformationEntry?.file || null,
      ),
    )
    .filter(Boolean);
  if (normalizedCandidateReferences.length === 0) {
    throw new Error(
      "Topics API did not return project file information entries for linking.",
    );
  }
  ...
  const scoredCandidates = normalizedCandidateReferences
    .map((topicFileReference) => {
      ...
      if (normalizedReference === selectedModelFileId) {
        ...
        normalizedReferenceLower.includes(normalizedSelectedFileIdLower)
      ) {
        matchScore += 700;
      }

      if (
        normalizedSelectedVersionIdLower &&
        normalizedReferenceLower.includes(normalizedSelectedVersionIdLower)
      ) {
        matchScore += 250;
      }
  ...
  const highestMatchScore = Math.max(
    ...scoredCandidates.map((scoredCandidate) => scoredCandidate.matchScore),
  );
  ...
  const resolvedTopicFileReference =
    highestScoredCandidates[0].topicFileReference;
...
None
*/
function buildTopicFileReferenceFromSelectedModel(
  selectedModelFile,
  checkReportData,
) {
  const selectedModelFileId = String(selectedModelFile?.fileId || "").trim();
  ...
  if (!selectedModelFileId || !selectedModelFileName) {
    throw new Error(
      "Selected IFC file id and name are required for topic-file linking.",
    );
  }

  const ifcProjectGlobalId = String(
    checkReportData?.modelContext?.ifcProjectGlobalId || "",
  ).trim();
...
async function resolveTopicFileReferenceForCurrentModel(
  ...
  const localTopicFileReference = buildTopicFileReferenceFromSelectedModel(
    selectedModelFile,
    checkReportData,
  );
  ...
  const projectFileInformationEntries =
    await loadProjectBcfFileInformationEntries(
      topicsApiBaseUrl,
      accessToken,
      projectId,
    );
  return resolveTopicFileReferenceForSelectedModel(
    projectFileInformationEntries,
    selectedModelFile,
  );
}
...
None
*/
function buildTopicPayloadForSelectedIssue(
  genericSettings,
  selectedIssueTopic,
) {
  ...
  const normalizedTopicStatus = String(
    normalizedGenericSettings.topicStatus || "",
  ).trim();
  const normalizedPriority = String(
    normalizedGenericSettings.priority || "",
  ).trim();
  const normalizedStage = String(normalizedGenericSettings.stage || "").trim();
  const normalizedDueDate = String(
    normalizedGenericSettings.dueDate || "",
  ).trim();

  const labelValues = collectUniqueTrimmedStringValues(
    Array.isArray(normalizedGenericSettings.labels)
      ? normalizedGenericSettings.labels
      : [],
  );
  const referenceLinks = collectUniqueTrimmedStringValues(
    normalizedGenericSettings.referenceLinks || [],
  );
...
None
*/
function buildPerspectiveCameraFromPlacementPoint(
  placementPoint,
  lengthUnitToMeters,
) {
  const normalizedPoint = convertPlacementPointToBcfMeters(
    placementPoint,
    lengthUnitToMeters,
  );
  if (!normalizedPoint) {
...
None
*/
function buildViewpointPayloadForSelectedIssue(
  selectedIssueTopic,
  usePlacementCamera,
  lengthUnitToMeters,
) {
      ...
      selection: [
        {
          ...
          authoring_tool_id: hasExpressId
            ? `ExpressId:${String(expressIdValue)}`
            : "Trimble-Lab",
        },
      ],
    },
...
*/
async function createSingleBcfTopic(
  topicsApiBaseUrl,
  accessToken,
  projectId,
  topicPayload,
    ...
    if (statusCode === 406 || statusCode === 415) {
      try {
        ...
        if (
          ...
          legacyStatusCode !== 406 &&
          legacyStatusCode !== 415
        ) {
...
*/
async function createSingleBcfViewpoint(
  topicsApiBaseUrl,
  accessToken,
  projectId,
  topicGuid,
...
*/
async function updateSingleBcfTopicFiles(
  topicsApiBaseUrl,
  accessToken,
  projectId,
  topicGuid,
...
async function createBcfTopicsFromModalRequest({
  ...
  const selectedTopics = Array.isArray(creationRequest?.topics)
    ? creationRequest.topics
    : [];
  for (const selectedTopic of selectedTopics) {
    try {
      const topicPayload = buildTopicPayloadForSelectedIssue(
        creationRequest?.generic || {},
        selectedTopic,
      );
      const createdTopicPayload = await createSingleBcfTopic(
        topicsApiBaseUrl,
        accessToken,
        projectId,
        topicPayload,
      ...
      if (!createdTopicGuid) {
        throw new Error(
          "Topics API did not return created topic guid, so file linking cannot continue.",
        );
      }

      const topicActions = collectUniqueTrimmedStringValues(
        createdTopicPayload?.authorization?.topic_actions,
      );
      if (topicFileReference) {
        const canUpdateFiles =
          topicActions.length === 0 || topicActions.includes("updateFiles");
        if (!canUpdateFiles) {
          throw new Error(
        ...
        await updateSingleBcfTopicFiles(
          topicsApiBaseUrl,
          accessToken,
          projectId,
          createdTopicGuid,
      ...
      const createViewpointsEnabled =
        creationRequest?.generic?.createViewpoints === true;
      if (!createViewpointsEnabled) {
        continue;
      }

      const canCreateViewpoint =
        topicActions.length === 0 || topicActions.includes("createViewpoint");
      ...
      await createSingleBcfViewpoint(
        topicsApiBaseUrl,
        accessToken,
        projectId,
        createdTopicGuid,
...
async function exportLatestCheckReportAsJson() {
  if (
    ...
    setStatus(
      "Wait until current operation finishes before exporting report.",
      "warning",
    );
    return;
  }
  ...
  if (!runtimeState.latestCheckResult || !runtimeState.downloadedIfcFile) {
    setStatus(
      "Run one checker execution before exporting report JSON.",
      "warning",
    );
    return;
  }
...
async function exportLatestBcfDraftAsJson() {
  ...
  if (
    ...
    setStatus(
      "Wait until current operation finishes before creating BCFs.",
      "warning",
    );
    return;
  }
  ...
  if (selectedIssueSelectors.length === 0) {
    ...
    setStatus(
      "Select at least one error or warning from report results before creating BCFs.",
      "warning",
    );
    return;
  }
  ...
  if (selectedIssueRows.length === 0) {
    setStatus(
      "Selected errors/warnings were not found in current report. Update selections and try again.",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    const topicsApiBaseUrl = await resolveTopicsApiBaseUrlForProject(
      accessToken,
    ...
    const canCreateTopics =
      extensionOptions.projectActions.length === 0 ||
      extensionOptions.projectActions.includes("createTopic");
    if (!canCreateTopics) {
      setStatus(
        "Current user does not have createTopic permission in Topics API project authorization.",
        "error",
      );
      return;
    }
    ...
    const canUpdateTopicFiles =
      extensionOptions.topicActions.length === 0 ||
      extensionOptions.topicActions.includes("updateFiles");
    ...
    const creationRequest = await openBcfTopicCreationModal(
      selectedIssueRows,
      extensionOptions,
    );
    if (!creationRequest) {
    ...
    if (
      !Array.isArray(creationRequest.topics) ||
      creationRequest.topics.length === 0
    ) {
      setStatus(
        "No topics were selected in modal. Create BCFs action was cancelled.",
        "warning",
      );
      return;
    }

    const bcfLengthUnitToMeters = resolveBcfLengthUnitToMetersFromCheckResult(
    ...
    setStatus(
      "Resolving selected model file reference for topic linking...",
      "info",
    );
    ...
    setStatus(
      `Creating ${creationRequest.topics.length} BCF topic(s) in Trimble Connect...`,
      "info",
    );
    ...
    if (creationResult.createdTopicCount <= 0) {
      const firstFailureMessage =
        creationResult.failedTopics[0]?.error || "No topics were created.";
...
async function downloadSelectedIfcFileToMemory() {
  ...
  if (!isNonEmptyString(coreApiBaseUrl)) {
    setStatus(
      "Core API host was not resolved yet. Refresh file list first.",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    setStatus(
      `Tarkastusvalintojen avaaminen epaonnistui: ${errorMessage}`,
      "error",
    );
    return;
  }
  ...
  try {
    ...
    setStatus(
      `Downloading ${selectedFileEntry.name} to browser memory...`,
      "info",
    );
    const downloadedBuffer = await downloadIfcToBrowserMemory(
      signedDownloadUrl,
      selectedFileEntry,
    );
...
function renderFileTable(fileEntries) {
  ...
  const hasPreviousSelection = normalizedEntries.some(
    (fileEntry) =>
      buildFileRowKey(fileEntry) === runtimeState.selectedFileRowKey,
  );
  ...
  normalizedEntries.forEach((fileEntry, fileIndex) => {
    ...
    tableRow.addEventListener("click", (clickEvent) => {
      if (
        clickEvent.target instanceof HTMLInputElement &&
        clickEvent.target.type === "radio"
      ) {
        return;
      }
...
async function loadAndRenderProjectFiles() {
  ...
  try {
    ...
    if (
      workspaceApi?.extension &&
      typeof workspaceApi.extension.setStatusMessage === "function"
    ) {
