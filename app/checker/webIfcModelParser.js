...
function resolveIfcLengthUnitToMeters(ifcApi, modelId, projectLine) {
  ...
  const unitReferences = Array.isArray(unitAssignmentLine?.Units)
    ? unitAssignmentLine.Units
    : [];
  ...
  for (const unitReference of unitReferences) {
    ...
    if (normalizedUnitName === "METRE") {
      ...
      return Number.isFinite(siPrefixScale) && siPrefixScale > 0
        ? siPrefixScale
        : 1;
    }
    ...
    const conversionFactorLine = safeGetLine(
      ifcApi,
      modelId,
      conversionFactorId,
    );
    ...
    const conversionValue = Number(
      unwrapStepValue(conversionFactorLine.ValueComponent),
    );
    ...
    if (Number.isInteger(unitComponentId) && unitComponentId > 0) {
      ...
      const componentUnitName = normalizeIfcEnumLiteral(
        unitComponentLine?.Name,
      );
    ...
    if (
      Number.isFinite(normalizedLengthUnitScale) &&
      normalizedLengthUnitScale > 0
    ) {
...
function inferIfcMatrixConvention(matrixValues) {
  const columnMajorAffineError =
    Math.abs(matrixValues[3]) +
    Math.abs(matrixValues[7]) +
    Math.abs(matrixValues[11]);
  const rowMajorAffineError =
    Math.abs(matrixValues[12]) +
    Math.abs(matrixValues[13]) +
    Math.abs(matrixValues[14]);
  return columnMajorAffineError <= rowMajorAffineError
    ? "column-major"
    : "row-major";
}
...
None
*/
function transformPointWithIfcMatrix(
  matrixValues,
  pointCandidate,
  matrixConvention,
) {
...
function loadIfcCoordinationTransform(ifcApi, modelId) {
  ...
  const normalizedCoordinationMatrix =
    normalizeIfcTransformationMatrix(coordinationMatrix);
...
None
*/
function readObjectPlacementPointFromWorldTransform(
  ifcApi,
  modelId,
  objectLine,
  ...
  try {
    worldTransformMatrix = ifcApi.GetWorldTransformMatrix(
      modelId,
      rootPlacementId,
    );
  } catch (error) {
  ...
  const normalizedWorldMatrix =
    normalizeIfcTransformationMatrix(worldTransformMatrix);
...
None
*/
function readApproximateObjectPlacementPointBySummingOffsets(
  ifcApi,
  modelId,
  objectLine,
) {
  ...
  while (Number.isInteger(currentPlacementId) && currentPlacementId > 0) {
    ...
    const relativePlacementId = unwrapStepValue(
      placementLine.RelativePlacement,
    );
    if (Number.isInteger(relativePlacementId) && relativePlacementId > 0) {
      const relativePlacementLine = safeGetLine(
        ifcApi,
        modelId,
        relativePlacementId,
      );
      ...
      if (Number.isInteger(locationId) && locationId > 0) {
        ...
        const locationPoint = extractCartesianPointFromCoordinatesPayload(
          locationLine?.Coordinates,
        );
        if (locationPoint) {
          resolvedPoint = {
...
None
*/
function readObjectPlacementPoint(
  ifcApi,
  modelId,
  objectLine,
  cachedCoordinationTransform,
  ...
  const approximatePoint = readApproximateObjectPlacementPointBySummingOffsets(
    ifcApi,
    modelId,
    objectLine,
  );
...
function collectIdsFromVector(idVector) {
  if (
    !idVector ||
    typeof idVector.size !== "function" ||
    typeof idVector.get !== "function"
  ) {
    return [];
  }
...
function decodeIfcTextPrefix(ifcBytes, maximumBytes = 8192) {
  ...
  const textSlice = ifcBytes.subarray(
    0,
    Math.min(ifcBytes.byteLength, maximumBytes),
  );
...
function validateIfcBinaryPayload(ifcBytes) {
  ...
  const hasZipSignature =
    ifcBytes.byteLength >= 2 && ifcBytes[0] === 0x50 && ifcBytes[1] === 0x4b;
  if (hasZipSignature) {
    return {
      isValid: false,
      reason:
        "Downloaded file is ZIP-compressed. This checker expects plain .ifc content.",
    };
  }
  ...
  if (
    ...
    return {
      isValid: false,
      reason:
        "Downloaded payload appears to be HTML error page, not IFC bytes.",
    };
  }
  ...
  if (!prefixText.includes("ISO-10303-21")) {
    return {
      isValid: false,
      reason:
        "IFC STEP header (ISO-10303-21) was not found in downloaded payload.",
    };
  }
...
function readPropertySet(ifcApi, modelId, propertySetLine, typeNameByCode) {
  const propertySetName =
    unwrapStepValue(propertySetLine.Name) || "Unnamed PropertySet";
  const hasProperties = Array.isArray(propertySetLine.HasProperties)
    ? propertySetLine.HasProperties
    : [];
  ...
  hasProperties.forEach((propertyRef) => {
    ...
    const rawTypeCode =
      nominalValue && typeof nominalValue === "object"
        ? nominalValue.type
        : null;
...
export async function getInitializedWebIfcRuntime() {
  ...
  if (!window.WebIFC || typeof window.WebIFC.IfcAPI !== "function") {
    throw new Error(
      "web-ifc runtime is not available. Ensure web-ifc-api-iife.js is loaded before app/main.js.",
    );
  }
...
export async function verifyIfcModelReadable(ifcArrayBuffer) {
  ...
  try {
    ...
    if (Number.isInteger(ifcProjectTypeCode)) {
      const projectLineIds = safeGetLineIdsWithType(
        ifcApi,
        modelId,
        ifcProjectTypeCode,
      );
      if (projectLineIds.length === 0) {
        throw new Error(
          "No IFCPROJECT entity was found in the selected payload.",
        );
      }
    }
  } finally {
...
export async function parseIfcModelForChecking(ifcArrayBuffer, ruleDataset) {
  const { ifcApi, typeNameByCode, typeCodeByName } =
    await getInitializedWebIfcRuntime();
  const ifcBytes = new Uint8Array(ifcArrayBuffer);
  const payloadValidation = validateIfcBinaryPayload(ifcBytes);
  if (!payloadValidation.isValid) {
    throw new Error(
      `Model payload validation failed: ${payloadValidation.reason}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `IFC parser could not open the selected model. ${
        openModelError instanceof Error
          ? openModelError.message
          : String(openModelError)
      }`,
    );
  }
  ...
  try {
    ...
    allLineIds.forEach((lineId) => {
      ...
      const relatingBuildingElement = unwrapStepValue(
        line?.RelatingBuildingElement,
      );
      if (
        relatingBuildingElement !== null &&
        relatingBuildingElement !== undefined
      ) {
    ...
    const definesByPropertiesTypeCode = typeCodeByName.get(
      "IFCRELDEFINESBYPROPERTIES",
    );
    const definesByTypeTypeCode = typeCodeByName.get("IFCRELDEFINESBYTYPE");

    if (definesByPropertiesTypeCode) {
      const relationIds = safeGetLineIdsWithType(
        ifcApi,
        modelId,
        definesByPropertiesTypeCode,
      );
      relationIds.forEach((relationId) => {
        const relationLine = safeGetLine(ifcApi, modelId, relationId);
        if (
          !relationLine ||
          !relationLine.RelatingPropertyDefinition ||
          !relationLine.RelatedObjects
        ) {
          return;
        }

        const propertySetExpressId = unwrapStepValue(
          relationLine.RelatingPropertyDefinition,
        );
        ...
        const propertySetLine = safeGetLine(
          ifcApi,
          modelId,
          propertySetExpressId,
        );
        if (!propertySetLine || !propertySetLine.HasProperties) {
          return;
        }

        const parsedPropertySet = readPropertySet(
          ifcApi,
          modelId,
          propertySetLine,
          typeNameByCode,
        );
        const relatedObjects = Array.isArray(relationLine.RelatedObjects)
          ? relationLine.RelatedObjects
          : [];
    ...
    if (definesByTypeTypeCode) {
      const relationIds = safeGetLineIdsWithType(
        ifcApi,
        modelId,
        definesByTypeTypeCode,
      );
      relationIds.forEach((relationId) => {
        const relationLine = safeGetLine(ifcApi, modelId, relationId);
        if (
          !relationLine ||
          !relationLine.RelatingType ||
          !relationLine.RelatedObjects
        ) {
          return;
        }
    ...
    const systemTypeNames = Array.isArray(ruleDataset.systemEntityTypeNames)
      ? ruleDataset.systemEntityTypeNames
      : [];
    const componentTypeNames = Array.isArray(
    ...
    const ifcProjectTypeCode =
      typeCodeByName.get("IFCPROJECT") || window.WebIFC?.IFCPROJECT;
    if (Number.isInteger(ifcProjectTypeCode)) {
      const projectLineIds = safeGetLineIdsWithType(
        ifcApi,
        modelId,
      ...
      if (projectLineIds.length > 0) {
        ...
        ifcProjectGlobalId = String(
          unwrapStepValue(projectLine?.GlobalId) || "",
        ).trim();
        ifcProjectName = String(
          unwrapStepValue(projectLine?.Name) || "",
        ).trim();
        const resolvedLengthUnitToMeters = resolveIfcLengthUnitToMeters(
          ifcApi,
          modelId,
          projectLine,
        );
        if (
          Number.isFinite(resolvedLengthUnitToMeters) &&
          resolvedLengthUnitToMeters > 0
        ) {
    ...
    const systemTypeCodes = new Set(
      systemTypeNames
        .map((typeName) => typeCodeByName.get(typeName))
        .filter(Boolean),
    );
    const componentTypeCodes = new Set(
      componentTypeNames
        .map((typeName) => typeCodeByName.get(typeName))
        .filter(Boolean),
    );
    const targetTypeCodes = Array.from(
      new Set([...systemTypeCodes, ...componentTypeCodes]),
    );

    const parsedObjects = [];
    const cachedCoordinationTransform = loadIfcCoordinationTransform(
      ifcApi,
      modelId,
    );
    ...
    targetTypeCodes.forEach((typeCode) => {
      ...
      entityIds.forEach((entityId) => {
        ...
        const typeLevelPsets = typeLevelId
          ? psetsByObjectId.get(typeLevelId) || []
          : [];
        ...
        const systemIdentifyValues = readIdentityValues(
          propertiesBySet,
          SYSTEM_IDENTIFY_LABELS,
          SYSTEM_GROUP_NAME,
        );

        const hasComponentPropertySet = Object.keys(propertiesBySet).some(
          (psetName) => psetName.startsWith(COMPONENT_GROUP_NAME),
        );
        const hasSystemPropertySet = Object.keys(propertiesBySet).some(
          (psetName) => psetName.startsWith(SYSTEM_GROUP_NAME),
        );
        ...
        parsedObjects.push({
          ...
          ifcTypeName:
            typeNameByCode.get(typeCode) || `UNKNOWN_IFC_${typeCode}`,
          typeCode,
          placementPoint: readObjectPlacementPoint(
            ifcApi,
            modelId,
            objectLine,
            cachedCoordinationTransform,
          ...
          isComponentCandidate:
            hasComponentPropertySet ||
            hasAnyIdentityValue(componentIdentifyValues) ||
            componentTypeCodes.has(typeCode),
          isSystemCandidate:
            hasSystemPropertySet ||
            hasAnyIdentityValue(systemIdentifyValues) ||
            systemTypeCodes.has(typeCode),
        });
      });
    });
