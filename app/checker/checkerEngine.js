...
function buildEffectivePropertyStates(dataset, propertyStateOverrides) {
  ...
  const explicitOverrides =
    propertyStateOverrides && typeof propertyStateOverrides === "object"
      ? propertyStateOverrides
      : {};

  [
    ...(dataset.productPropertyDefinitions || []),
    ...(dataset.systemPropertyDefinitions || []),
  ].forEach((definition) => {
    ...
    const explicitState =
      explicitOverrides[definition.group]?.[definition.label];
    effectiveStates[definition.group][definition.label] =
      explicitState || overrideState || "checked";
  });
...
function buildComponentAllowedValueSets(productCodeDefinitions) {
  const safeProductCodeDefinitions = Array.isArray(productCodeDefinitions)
    ? productCodeDefinitions
    : [];

  return {
    "01 Komponentin pääryhmä": new Set(
      safeProductCodeDefinitions
        .map((entry) => entry?.hierarchy?.[1])
        .filter(Boolean),
    ),
    "02 Komponentin alaryhmä": new Set(
      safeProductCodeDefinitions
        .map((entry) => entry?.hierarchy?.[2])
        .filter(Boolean),
    ),
    "03 Komponentin koodi": new Set(
      safeProductCodeDefinitions.map((entry) => entry?.id).filter(Boolean),
    ),
    "04 Komponentin yleisnimi": new Set(
      safeProductCodeDefinitions.map((entry) => entry?.name).filter(Boolean),
    ),
    "05 Komponentin yleistunnus": new Set(
      safeProductCodeDefinitions
        .map((entry) => entry?.generalId)
        .filter(Boolean),
    ),
  };
}
...
function buildSystemAllowedValueSets(systemCodeDefinitions) {
  const safeSystemCodeDefinitions = Array.isArray(systemCodeDefinitions)
    ? systemCodeDefinitions
    : [];

  return {
    "01 Järjestelmälaji": new Set(
      safeSystemCodeDefinitions
        .map((entry) => entry?.hierarchy?.[1])
        .filter(Boolean),
    ),
    "02 Järjestelmäluokka": new Set(
      safeSystemCodeDefinitions
        .map((entry) => entry?.hierarchy?.[2])
        .filter(Boolean),
    ),
    "03 Järjestelmätyypin koodi": new Set(
      safeSystemCodeDefinitions.map((entry) => entry?.id).filter(Boolean),
    ),
    "04 Järjestelmätyyppi": new Set(
      safeSystemCodeDefinitions.map((entry) => entry?.name).filter(Boolean),
    ),
    "05 Järjestelmätyypin yleistunnus": new Set(
      safeSystemCodeDefinitions
        .map((entry) => entry?.generalId)
        .filter(Boolean),
    ),
...
function buildIdentifyStatusMap(identifyLabels, identifyValues, allowedSets) {
  const statusMap = {};
  const safeIdentifyValues =
    identifyValues && typeof identifyValues === "object" ? identifyValues : {};
  const safeAllowedSets =
    allowedSets && typeof allowedSets === "object" ? allowedSets : {};

  identifyLabels.forEach((label) => {
    const value = safeIdentifyValues[label];
    const allowedSet =
      safeAllowedSets[label] instanceof Set
        ? safeAllowedSets[label]
        : new Set();
...
function evaluateRequiredProperties(
  ...
  const safePropertiesBySet =
    propertiesBySet && typeof propertiesBySet === "object"
      ? propertiesBySet
      : {};
  const safeCoveringElementIds =
    coveringElementIds instanceof Set ? coveringElementIds : new Set();
  ...
  requiredPropertyIds.forEach((propertyId) => {
    ...
    const propertyState =
      effectivePropertyStates[propertyDefinition.group]?.[
    ...
    const resolvedProperty =
      safePropertiesBySet[propertyDefinition.group]?.[propertyDefinition.label];
    const isIdentifyProperty = identifyLabels.includes(
      propertyDefinition.label,
    );
    ...
    if (
      propertyDefinition.ifcPropertyType &&
      resolvedProperty.rawType !==
        propertyDefinition.ifcPropertyType.toUpperCase()
    ) {
      issues.push({
        type: "wrong_type",
        severity: "warning",
  ...
  return (
    (Array.isArray(systemCodeDefinitions) ? systemCodeDefinitions : []).find(
      (entry) =>
        ...
        entry.generalId ===
          safeIdentifyValues["05 Järjestelmätyypin yleistunnus"],
    ) || null
  );
}
...
export function runModelChecks(
  ...
  const safeRuleDataset =
    ruleDataset && typeof ruleDataset === "object" ? ruleDataset : {};
  const safeParsedModel =
    parsedModel && typeof parsedModel === "object" ? parsedModel : {};
  const parsedObjects = Array.isArray(safeParsedModel.parsedObjects)
    ? safeParsedModel.parsedObjects
    : [];
  const coveringElementIds =
    safeParsedModel.coveringElementIds instanceof Set
      ? safeParsedModel.coveringElementIds
      : new Set();

  const productCodeDefinitions = Array.isArray(
    safeRuleDataset.productCodeDefinitions,
  )
    ? safeRuleDataset.productCodeDefinitions
    : [];
  const systemCodeDefinitions = Array.isArray(
    safeRuleDataset.systemCodeDefinitions,
  )
    ? safeRuleDataset.systemCodeDefinitions
    : [];

  const productPropertyById = mapPropertyDefinitionsById(
    safeRuleDataset.productPropertyDefinitions || [],
  );
  const systemPropertyById = mapPropertyDefinitionsById(
    safeRuleDataset.systemPropertyDefinitions || [],
  );
  ...
  const isProductIdentificationEnabled =
    isProductScopeEnabled && enabledCheckPhases.productIdentification !== false;
  const isProductContentEnabled =
    isProductScopeEnabled && enabledCheckPhases.productContent !== false;
  const isSystemIdentificationEnabled =
    isSystemScopeEnabled && enabledCheckPhases.systemIdentification !== false;
  const isSystemContentEnabled =
    isSystemScopeEnabled && enabledCheckPhases.systemContent !== false;
  ...
  const componentAllowedSets = buildComponentAllowedValueSets(
    productCodeDefinitions,
  );
  const systemAllowedSets = buildSystemAllowedValueSets(systemCodeDefinitions);
  const normalizedModelContext = {
    ifcProjectGlobalId: String(
      safeParsedModel?.modelContext?.ifcProjectGlobalId || "",
    ).trim(),
    ifcProjectName: String(
      safeParsedModel?.modelContext?.ifcProjectName || "",
    ).trim(),
  ...
  parsedObjects.forEach((parsedObject) => {
    const normalizedParsedObject =
      parsedObject && typeof parsedObject === "object" ? parsedObject : {};
    const normalizedComponentIdentifyValues =
      normalizedParsedObject.componentIdentifyValues &&
      typeof normalizedParsedObject.componentIdentifyValues === "object"
        ? normalizedParsedObject.componentIdentifyValues
        : {};
    const normalizedSystemIdentifyValues =
      normalizedParsedObject.systemIdentifyValues &&
      typeof normalizedParsedObject.systemIdentifyValues === "object"
        ? normalizedParsedObject.systemIdentifyValues
        : {};
    const normalizedPropertiesBySet =
      normalizedParsedObject.propertiesBySet &&
      typeof normalizedParsedObject.propertiesBySet === "object"
        ? normalizedParsedObject.propertiesBySet
        : {};

    const cleanObjectName =
      normalizedParsedObject.objectName &&
      normalizedParsedObject.objectName.trim() !== ""
        ? normalizedParsedObject.objectName
        : "Unnamed";
    ...
    if (isProductScopeEnabled && normalizedParsedObject.isComponentCandidate) {
      ...
      const matchedProductCode = findMatchingProductCode(
        productCodeDefinitions,
      ...
      if (matchedProductCode) {
        ...
        if (isProductContentEnabled) {
          ...
          const hasContentErrors = issues.some(
          ...
          if (issues.length > 0) {
            productFindings.push({
              ...
              issues,
              level3Key:
                normalizedComponentIdentifyValues["04 Komponentin yleisnimi"] ||
                "Not defined",
    ...
    if (isSystemScopeEnabled && normalizedParsedObject.isSystemCandidate) {
      ...
      const matchedSystemCode = findMatchingSystemCode(
        systemCodeDefinitions,
        ...
        if (isSystemContentEnabled) {
          ...
          if (issues.length > 0) {
            systemFindings.push({
              ...
              issues,
              level3Key:
                normalizedSystemIdentifyValues["04 Järjestelmätyyppi"] ||
                "Not defined",
              isUnidentified: false,
              identifyProps: identifyStatus,
              ifcEntity: normalizedParsedObject.ifcTypeName,
  ...
  const totalIssues = [...productFindings, ...systemFindings].reduce(
    (sum, finding) => sum + finding.issues.length,
    0,
  );
  const errorCount = [...productFindings, ...systemFindings].reduce(
    (sum, finding) =>
      sum + finding.issues.filter((issue) => issue.severity === "error").length,
    0,
  );
  const warningCount = [...productFindings, ...systemFindings].reduce(
    (sum, finding) =>
      sum +
      finding.issues.filter((issue) => issue.severity === "warning").length,
    0,
  );
