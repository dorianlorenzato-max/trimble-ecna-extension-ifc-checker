...
function readIdentifierFromUrlQuery(fileUrlString, parameterNames) {
  if (
    !isNonEmptyString(fileUrlString) ||
...
function isDetailViewerUrl(urlString) {
  ...
  return (
    normalizedCandidate.includes("/detailviewer") ||
    normalizedCandidate.includes("detailviewer?")
  );
}
...
function buildNormalizedFileDescriptor(eventArguments) {
  ...
  const normalizedId = pickFirstIdentifier([
    ...
    readIdentifierFromUrlQuery(downloadLink, [
      "fileId",
      "modelId",
      "id",
      "sourceId",
...
export function isIfcFileSelection(selectedFile) {
  ...
  const typeHints = [
    ...
    .map((candidateHint) =>
      typeof candidateHint === "string" ? candidateHint.toLowerCase() : "",
    )
    .filter(Boolean);
...
function decodeTextPrefix(binaryData, maximumBytes = 8192) {
  ...
  const probeSlice = new Uint8Array(
    binaryData,
    0,
    Math.min(maximumBytes, binaryData.byteLength),
  );
...
function validateIfcPayload(binaryData, contentType = "") {
  ...
  const hasZipSignature =
    bytes.byteLength >= 2 && bytes[0] === 0x50 && bytes[1] === 0x4b;
  if (hasZipSignature) {
    return {
      isValid: false,
      reason:
        "Downloaded file is ZIP-compressed. Provide plain .ifc content for web-ifc parser.",
    };
  }
  ...
  if (!prefixText.includes("ISO-10303-21")) {
    return {
      isValid: false,
      reason:
        "Downloaded payload does not contain IFC STEP header (ISO-10303-21).",
    };
  }
...
None
*/
function buildCoreMetadataEndpointCandidates(
  coreApiBaseUrl,
  projectId,
  fileId,
  versionId,
  ...
  if (isNonEmptyString(versionId)) {
    endpointCandidates.unshift(
      `${coreApiBaseUrl}/projects/${projectId}/files/${fileId}/versions/${versionId}`,
    );
    endpointCandidates.push(
      `${coreApiBaseUrl}/files/${fileId}/versions/${versionId}`,
    );
  }
...
None
*/
function buildCoreBinaryEndpointCandidates(
  coreApiBaseUrl,
  projectId,
  fileId,
  versionId,
  ...
  if (isNonEmptyString(versionId)) {
    endpointCandidates.unshift(
      `${coreApiBaseUrl}/projects/${projectId}/files/${fileId}/versions/${versionId}/download`,
    );
    endpointCandidates.push(
      `${coreApiBaseUrl}/projects/${projectId}/files/${fileId}/versions/${versionId}/content`,
    );
    endpointCandidates.push(
      `${coreApiBaseUrl}/files/${fileId}/versions/${versionId}/download`,
    );
  }
...
*/
async function tryDownloadViaFileIdCoreApi(
  selectedFile,
  projectId,
  accessToken,
) {
  ...
  if (
    ...
    diagnostics.push(
      "Core API id-based fallback was skipped because projectId, fileId or access token was missing.",
    );
    return {
      ok: false,
      data: null,
  ...
  const binaryHeaders = buildRequestHeaders(
    accessToken,
    "application/octet-stream,*/*",
  );

  for (const coreApiBaseUrl of coreApiBaseUrls) {
    const metadataEndpoints = buildCoreMetadataEndpointCandidates(
      coreApiBaseUrl,
      projectId,
      fileId,
      versionId,
    ...
    for (const metadataEndpoint of metadataEndpoints) {
      const metadataAttempt = await tryReadJson(
        metadataEndpoint,
        jsonHeaders,
        "omit",
      );
      diagnostics.push(
        `Metadata ${metadataEndpoint} -> ${metadataAttempt.status || "network-error"}`,
      );
      ...
      const discoveredDownloadUrl = pickDownloadUrlFromMetadata(
      ...
      const downloadAttempt = await tryDownload(
        resolvedDownloadUrl,
        binaryHeaders,
        "omit",
      );
      diagnostics.push(
        `Metadata URL download ${resolvedDownloadUrl} -> ${downloadAttempt.status || "network-error"}`,
      );

      if (downloadAttempt.ok) {
        const payloadValidation = validateIfcPayload(
          downloadAttempt.data,
          downloadAttempt.contentType,
        );
        if (!payloadValidation.isValid) {
          diagnostics.push(
            `Metadata URL payload rejected: ${payloadValidation.reason}`,
          );
          continue;
        }
    ...
    const binaryEndpoints = buildCoreBinaryEndpointCandidates(
      coreApiBaseUrl,
      projectId,
      fileId,
      versionId,
    ...
    for (const binaryEndpoint of binaryEndpoints) {
      const binaryAttempt = await tryDownload(
        binaryEndpoint,
        binaryHeaders,
        "omit",
      );
      diagnostics.push(
        `Binary ${binaryEndpoint} -> ${binaryAttempt.status || "network-error"}`,
      );
      ...
      if (binaryAttempt.ok) {
        ...
        if (!payloadValidation.isValid) {
          diagnostics.push(
            `Binary payload rejected: ${payloadValidation.reason}`,
          );
          continue;
        }
...
async function tryDirectLinkDownload(resolvedDownloadUrl, accessToken) {
  const diagnostics = [];
  const authorizedHeaders = buildRequestHeaders(
    accessToken,
    "application/octet-stream,*/*",
  );

  const authorizedAttempt = await tryDownload(
    resolvedDownloadUrl,
    authorizedHeaders,
    "omit",
  );
  diagnostics.push(
    `Direct authorized ${resolvedDownloadUrl} -> ${
      authorizedAttempt.status ||
      authorizedAttempt.errorMessage ||
      "network-error"
    }`,
  );
  if (authorizedAttempt.ok) {
    const payloadValidation = validateIfcPayload(
      authorizedAttempt.data,
      authorizedAttempt.contentType,
    );
    if (!payloadValidation.isValid) {
      diagnostics.push(
        `Direct authorized payload rejected: ${payloadValidation.reason}`,
      );
    } else {
      return {
        ok: true,
  ...
  if (fallbackAttempt.ok) {
    const payloadValidation = validateIfcPayload(
      ...
      fallbackAttempt.contentType,
    );
    if (!payloadValidation.isValid) {
      diagnostics.push(
        `Direct fallback payload rejected: ${payloadValidation.reason}`,
      );
      return {
        ok: false,
        data: null,
...
function formatDiagnosticsSummary(diagnostics) {
  ...
  const diagnosticsSuffix = hasMoreDiagnostics
    ? ` | ... (+${diagnostics.length - diagnosticsLimit} more attempts)`
    : "";
...
function canUseCoreApiFallbackFromCurrentOrigin() {
  ...
  return (
    currentHostName === "connect.trimble.com" ||
    currentHostName.endsWith(".connect.trimble.com")
  );
}
...
*/
export async function downloadSelectedIfcAsArrayBuffer(
  selectedFile,
  accessToken,
  projectId = "",
) {
  ...
  const hasNonDetailViewerLink =
    isNonEmptyString(resolvedLinkUrl) && !isDetailViewerUrl(resolvedLinkUrl);

  if (hasNonDetailViewerLink) {
    const directAttempt = await tryDirectLinkDownload(
      resolvedLinkUrl,
      accessToken,
    );
    ...
    collectedDiagnostics.push(
      "Selected file link points to detailviewer UI route, not direct IFC bytes.",
    );
  }

  if (canUseCoreApiFallbackFromCurrentOrigin()) {
    const coreApiAttempt = await tryDownloadViaFileIdCoreApi(
      selectedFile,
      projectId,
      accessToken,
    );
  ...
  if (isNonEmptyString(resolvedLinkUrl) && !hasNonDetailViewerLink) {
    const lastResortDirectAttempt = await tryDirectLinkDownload(
      resolvedLinkUrl,
      accessToken,
    );
  ...
  throw new Error(
    `IFC download failed for file id "${selectedFile.id || "unknown"}". ${formatDiagnosticsSummary(
