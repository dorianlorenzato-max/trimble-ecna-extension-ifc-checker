...
function normalizeTimestampToIsoString(timestampCandidate) {
  const candidateDate =
    timestampCandidate instanceof Date
      ? timestampCandidate
      : new Date(timestampCandidate || Date.now());
...
function collectIssueRowsFromReport(reportData, selectedIssueLookup = null) {
  const normalizedFindings = [
    ...(Array.isArray(reportData?.productFindings)
      ? reportData.productFindings.map((finding) => ({
          scope: "product",
          finding,
        }))
      : []),
    ...(Array.isArray(reportData?.systemFindings)
      ? reportData.systemFindings.map((finding) => ({
          scope: "system",
          finding,
        }))
      : []),
  ...
  normalizedFindings.forEach(({ scope, finding }) => {
    ...
    if (findingIssues.length === 0 && finding?.isUnidentified) {
      ...
      if (
        selectedIssueLookup &&
        !selectedIssueLookup.has(syntheticSelectionKey)
      ) {
        return;
      }
...
export function buildCheckerReportExportPayload({
  ...
  const generatedAtIso = normalizeTimestampToIsoString(generatedAt);
  const productFindings = Array.isArray(reportData?.productFindings)
    ? reportData.productFindings
    : [];
  const systemFindings = Array.isArray(reportData?.systemFindings)
    ? reportData.systemFindings
    : [];
  const unknownFindings = Array.isArray(reportData?.unknownFindings)
    ? reportData.unknownFindings
    : [];
...
export function buildBcfDraftPayload({
  ...
  const errorTopics = topics.filter((topic) =>
    topic.labels.includes("error"),
  ).length;
  const warningTopics = topics.filter((topic) =>
    topic.labels.includes("warning"),
  ).length;
...
export function triggerJsonFileDownload(fileName, payload) {
  ...
  const rawBaseName = rawFileName.toLowerCase().endsWith(".json")
